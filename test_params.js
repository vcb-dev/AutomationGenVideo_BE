...
async function run() {
    ...
    try {
        ...
        console.log('Testing with text_field_as_key: true');
        try {
            const res = await axios.get(`https://open.larksuite.com/open-apis/bitable/v1/apps/${BASE_ID}/tables/${TABLE_ID}/records`, {
                headers: { Authorization: `Bearer ${token}` },
                params: {
                    page_size: 20,
                    text_field_as_key: true // This is what the service uses
                }
            });
            console.log('Success! Count:', res.data.data.items?.length);
        } catch (e) {
            console.error('Error with text_field_as_key:', e.response?.data?.msg || e.message);
        }
    } catch (err) {

 ...
 *
 * Map 3 field: title (cột CHỦ ĐỀ), script (cột KỊCH BẢN CHI TIẾT) và content_line_id
 * (cột TUYẾN — A1..A5), tra theo tên trong bảng content_lines. Các cột còn lại trong CSV
 * (ID, LINK, NHÓM CHỦ ĐỀ, Trạng thái, Các mục mẹ) không có field tương ứng nên bỏ qua.
 *
 * ContentLine A1..A5 phải tồn tại sẵn trong DB đích (bảng content_lines, dùng chung mọi
 * team) trước khi chạy — script sẽ báo lỗi và dừng nếu thiếu.
 *
 ...
 *
 * Idempotent: title chưa tồn tại → insert mới (kèm content_line_id); title đã tồn tại
 * sẵn nhưng chưa có content_line_id → backfill content_line_id; chạy lại nhiều lần an toàn.
 *
...
const DATA: { title: string; script: string | null; contentLine: string }[] = [
  {
...
const CONTENT_LINE_NAMES = ['A1', 'A2', 'A3', 'A4', 'A5'];
...
async function main() {
  ...
  const contentLines = await prisma.contentLine.findMany({
    where: { name: { in: CONTENT_LINE_NAMES } },
  });
  const contentLineIdByName = new Map(contentLines.map((l) => [l.name, l.id]));
  const missingLines = CONTENT_LINE_NAMES.filter((n) => !contentLineIdByName.has(n));
  if (missingLines.length > 0) {
    throw new Error(`Thiếu ContentLine trong DB: ${missingLines.join(', ')}. Tạo trước khi chạy seed này.`);
  }

  const existing = await prisma.teamContent.findMany({
    where: { team_id: team.id },
    select: { id: true, title: true, content_line_id: true },
  });
  const existingByTitle = new Map(existing.map((c) => [c.title, c]));

  const toInsert = DATA.filter((r) => !existingByTitle.has(r.title));
  const toBackfill = DATA.filter((r) => {
    const found = existingByTitle.get(r.title);
    return found && !found.content_line_id;
  });
  ...
  if (toInsert.length > 0) {
    const result = await prisma.teamContent.createMany({
      data: toInsert.map((r) => ({
        ...
        title: r.title,
        script: r.script,
        content_line_id: contentLineIdByName.get(r.contentLine),
  ...
  for (const r of toBackfill) {
    const found = existingByTitle.get(r.title)!;
    await prisma.teamContent.update({
      where: { id: found.id },
      data: { content_line_id: contentLineIdByName.get(r.contentLine) },
    });
  }
  if (toBackfill.length > 0) {
    console.log(`Backfilled content_line_id cho ${toBackfill.length} record đã tồn tại sẵn.`);
  }

  const untouched = DATA.length - toInsert.length - toBackfill.length;
  console.log(`Tổng: ${DATA.length} | Insert mới: ${toInsert.length} | Backfill tuyến: ${toBackfill.length} | Bỏ qua (đã có sẵn tuyến): ${untouched}`);
}

...
async function runDeepAuthDiagnostics() {
  console.log('================================================================');
  console.log('🔬 DEEP AUTH NETWORK & APPLICATION DIAGNOSTIC TEST SUITE');
  console.log('================================================================\n');
  ...
  try {
    ...
    console.log(`✓ Status Code: ${regRes.status} (Expected: 201)`);
    ...
    // -------------------------------------------------------------
    // TEST 2: Inactive User Login Rejection (401)
    // -------------------------------------------------------------
    ...
    console.log('✓ Cookies Received:', Object.keys(setCookies));
    ...
    if (dbUserAfterLogin.refresh_token_hash !== expectedRtHash) {
      throw new Error('DB refresh_token_hash does not match the issued refresh token hash!');
    }
    ...
    const newAtCookie = newCookies['vcbi_at'];
    ...
    console.error('❌ DIAGNOSTIC TEST FAILED:', error.response?.data || error.message);
    throw error;
  } finally {
    // Cleanup test user
    if (testUserId) {
...
runDeepAuthDiagnostics().catch(() => process.exit(1));

...
async function main() {
    // 1. ALL Chung Đỗ records in lark_kpi - check team per month
    const chungKpis = await db.larkKPI.findMany({
    ...
    console.log(`\n=== ALL lark_kpi for "Chung" (${chungKpis.length} rows) ===`);
    ...
    // 2. All leaders in lark_kpi for Global-JP1
    const jp1Leaders = await db.larkKPI.findMany({
        where: { team: { contains: 'JP1', mode: 'insensitive' } },
        select: { name: true, team: true, employee_id: true, month: true, state: true },
        distinct: ['name']
    });
    console.log(`\n=== Distinct people in lark_kpi with team containing "JP1" (${jp1Leaders.length}) ===`);
    ...
    const jp1Count = await db.larkKPI.count({ where: { team: { contains: 'JP1', mode: 'insensitive' } } });
    const daiLoanCount = await db.larkKPI.count({ where: { team: { contains: 'Đài Loan', mode: 'insensitive' } } });
    const globalDLCount = await db.larkKPI.count({ where: { team: { contains: 'Global Đài', mode: 'insensitive' } } });
    console.log(`\n=== KPI counts by team ===`);

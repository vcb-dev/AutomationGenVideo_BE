...
export class TaskAutoTasksService {
  ...
  async create(dto: CreateTaskDto, creatorId: string, roles: string[] = []) {
    ...
    // MEMBER (editor) chỉ được tự tạo task cho chính mình
    if (!isPrivileged) {
    ...
    if (!isPrivileged) {
      ...
      if (!membership || membership.team_id !== dto.team_id) {
  ...
  async update(
    ...
    if (dto.status === "CANCELLED" && !isPrivileged) {
      throw new ForbiddenException(
        "Only ADMIN/MANAGER/LEADER can cancel tasks",
      );
    }
  ...
  private async getLeaderDashboard(leaderId: string) {
    ...
    const kpiByUser = Object.fromEntries(
      editorKpis.map((k) => [k.user_id, k]),
    );
  ...
  private async getPersonalDashboard(userId: string) {
    ...
    const [tasksByStatus, todayDeadline, overdue, myKpi] = await Promise.all([
      ...
      this.prisma.editorKpi.findUnique({

...
export class InstagramScraperService {
  ...
  async scrapeProfileReels(
    ...
    try {
      ...
      for (const r of reels) {
        if (cutoffDate && r.date_posted && new Date(r.date_posted) < cutoffDate) {
          continue;
        }
  ...
  async toggleProfile(
    ...
    await this.prisma.scraperInstagramProfile.update({ where: { id }, data: updateData });
    return newValue;
  }
  ...
  async updateClassification(id: bigint, channel_type: string, product_lines: string[]) {
    return this.prisma.scraperInstagramProfile.update({
      where: { id },

...
/** Độ phân giải tối thiểu Reels chấp nhận (docs Meta) */
...
export function validateVideoForPublish(
  probe: MediaProbe,
  opts: {
    limits?: DurationLimits | null;
    requireAudio: boolean;
    label: string;
    /**
     * Bài này sẽ đăng dạng Reel. Khi đó khung hình dọc và độ phân giải tối thiểu
     * là BẮT BUỘC — nền tảng từ chối thẳng, nên chặn sớm với thông báo rõ vẫn hơn
     * để người dùng chờ rồi nhận lỗi mơ hồ từ Meta.
     */
    isReel?: boolean;
  },
): string[] {
  ...
  if (opts.isReel && probe.width && probe.height) {
    if (probe.width > probe.height) {
      errors.push(
        `${opts.label}: video KHỔ NGANG ${probe.width}x${probe.height}. Reels là định dạng ` +
        `dọc — cần 9:16 (ví dụ 1080x1920). Xuất lại theo khung dọc rồi đăng.`,
      );
    } else if (Math.abs(probe.width / probe.height - REELS_ASPECT_RATIO) > ASPECT_TOLERANCE) {
      errors.push(
        `${opts.label}: tỉ lệ ${probe.width}x${probe.height} không phải 9:16. Reels cần ` +
        `tỉ lệ dọc 9:16 (ví dụ 1080x1920).`,
      );
    }

    if (probe.width < REELS_MIN_WIDTH || probe.height < REELS_MIN_HEIGHT) {
      errors.push(
        `${opts.label}: độ phân giải ${probe.width}x${probe.height} thấp hơn mức tối thiểu ` +
        `${REELS_MIN_WIDTH}x${REELS_MIN_HEIGHT} của Reels.`,
      );
    }
  }
...
export function collectWarnings(probe: MediaProbe, label: string): string[] {
  ...
  if (probe.width && probe.height) {
    ...
    if (probe.width < REELS_MIN_WIDTH || probe.height < REELS_MIN_HEIGHT) {
      warnings.push(
        `${label}: độ phân giải ${probe.width}x${probe.height} thấp hơn mức tối thiểu ${REELS_MIN_WIDTH}x${REELS_MIN_HEIGHT}.`,
      );
    }
  }

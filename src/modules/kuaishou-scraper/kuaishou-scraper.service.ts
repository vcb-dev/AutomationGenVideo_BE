...
export class KuaishouScraperService {
  ...
  async toggleProfile(
    ...
    await this.prisma.scraperKuaishouProfile.update({ where: { id }, data: updateData });
    return newValue;
  }
  ...
  async periodicRefresh(options?: {
    ...
    if (profiles.length === 0) {
      this.logger.log(`[KUAISHOU-PERIODIC] Không có profile (${scope}) nào cần cào định kỳ.`);
      return { total: 0, done: 0, failed: 0 };
    }

    this.logger.log(`═══ [KUAISHOU-PERIODIC] Cào video mới cho ${profiles.length} profile(s) (scope: ${scope}, mode: ${options?.mode || 'default'}) ═══`);
    let done = 0;
    let failed = 0;
  ...
  async updateClassification(id: bigint, channel_type: string, product_lines: string[]) {
    return this.prisma.scraperKuaishouProfile.update({
      where: { id },

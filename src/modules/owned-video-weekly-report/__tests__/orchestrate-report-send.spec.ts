...
const DAT_NGUONG = 1_200_000;
const DUOI_NGUONG = 5_000;
...
function buildService({ videos = [video()], openId = 'ou_nguoi_nhan', nguong, sendMessage }: Phu = {}) {
  ...
  const configService = {
    get: (k: string) => {
      ...
      if (k === 'BAO_CAO_TUAN_NGUONG_VIEW') return nguong;
      return undefined;
    },
  } as any;
...
describe('ngưỡng view', () => {
  ...
  it('đọc đè từ BAO_CAO_TUAN_NGUONG_VIEW', () => {
    expect(buildService({ nguong: '500000' }).service.viewThreshold).toBe(500_000);
  });

  it('giá trị rác thì quay về mặc định, không để ngưỡng thành NaN rồi lọt hết video', () => {
    expect(buildService({ nguong: 'abc' }).service.viewThreshold).toBe(DEFAULT_VIEW_THRESHOLD);
    expect(buildService({ nguong: '-5' }).service.viewThreshold).toBe(DEFAULT_VIEW_THRESHOLD);
  });
});

describe('run — chỉ báo video vượt ngưỡng', () => {
  it('không video nào đạt thì KHÔNG gửi tin nào', async () => {
    const { service, send } = buildService({ videos: [video({ view_count: DUOI_NGUONG })] });

    const kq = await service.run(false);

    expect(kq.soVideoDat).toBe(0);
    expect(kq.messageContent).toBeNull();
    expect(kq.daGui).toBe(false);
    expect(send).not.toHaveBeenCalled();
  });

  it('video dưới ngưỡng bị chốt duoi_nguong — ngày thứ 8 không xét lại', async () => {
    const { service, record } = buildService({ videos: [video({ view_count: DUOI_NGUONG })] });
  ...
  it('chỉ ghi da_gui cho video ĐẠT ngưỡng, không ghi cho cả lô', async () => {
    const { service, upsert, send } = buildService({
      videos: [video({ view_count: DAT_NGUONG }), video({ view_count: DUOI_NGUONG }), video({ view_count: DUOI_NGUONG })],
    });

    const kq = await service.run(false);

    expect(kq.soVideoXet).toBe(3);
    expect(kq.soVideoDat).toBe(1);
...
describe('run — chế độ khô', () => {
  it('dựng được tin nhưng KHÔNG gửi và KHÔNG ghi nhật ký, kể cả bản ghi duoi_nguong', async () => {
    const { service, upsert, send } = buildService({
      videos: [video({ view_count: DAT_NGUONG }), video({ view_count: DUOI_NGUONG })],
    });

    const kq = await service.run(true);

    expect(kq.messageContent).toContain('view trong 7 ngày đầu');
    expect(kq.daGui).toBe(false);
    expect(send).not.toHaveBeenCalled();
    expect(upsert).not.toHaveBeenCalled();
  });
...
describe('run — xử lý lỗi gửi', () => {
  ...
  it('gửi hỏng vẫn trả về kết quả kèm lý do, không ném ra ngoài', async () => {
    ...
    await expect(service.run(false)).resolves.toMatchObject({ daGui: false });
  });
});

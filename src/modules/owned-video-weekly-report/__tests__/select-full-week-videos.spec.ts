...
describe('filterByThreshold — chỉ báo video đã bùng nổ', () => {
  ...
  it('tách đúng hai nhóm quanh mốc, đạt mốc thì tính là đạt', () => {
    const { dat, belowThreshold } = filterByThreshold(
      [v(999_999), v(1_000_000), v(1_330_000)],
      DEFAULT_VIEW_THRESHOLD,
    );
    expect(dat.map((x) => x.view_count)).toEqual([1_000_000, 1_330_000]);
  ...
  it('hạ ngưỡng thì nhiều video lọt hơn', () => {
    ...
    expect(filterByThreshold(lo, 1_000_000).dat).toHaveLength(1);
    expect(filterByThreshold(lo, 500_000).dat).toHaveLength(2);
    expect(filterByThreshold(lo, 100_000).dat).toHaveLength(3);
  });
});

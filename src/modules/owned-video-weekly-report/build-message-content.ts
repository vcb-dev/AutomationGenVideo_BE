/**
 * Dựng nội dung tin cảnh báo video bùng nổ. Hàm THUẦN — không chạm Prisma, không gọi mạng.
 *
...
/**
 * Trần số video liệt kê trong một tin.
 *
 * Đo thật: với ngưỡng 1 triệu chỉ có ~1 video/tuần đạt, nên bình thường tin chỉ có 1–2 khối và
 ...
 * "764K" là đủ. Đo trên tin thật dựng từ 1.037 video mới thấy.
 */
export function compactNumber(n: number): string {
  const format = (giaTri: number, hauTo: string) => {
    const soLe = giaTri >= 100 ? 0 : giaTri >= 10 ? 1 : 2;
    const factor = 10 ** soLe;
    const round2 = Math.round(giaTri * factor) / factor;
    return `${String(round2).replace('.', ',')}${hauTo}`;
  };
 ...
 * ngày sẽ rơi vào trường hợp này. Gửi tin báo "không có gì" mỗi sáng là cách nhanh nhất để
 * người nhận tắt thông báo, và thế là mất luôn cái tin thật sự đáng đọc.
 */
export function buildMessageContent(videos: FullWeekVideo[], nguong: number): string | null {
  ...
  const dong: string[] = [
    `🚀 Video vượt ${compactNumber(nguong)} view trong 7 ngày đầu`,

 ...
 * khoảng 1 video/tuần, phần lớn các ngày sẽ không có tin nào. Ngưỡng thấp hơn cho tần suất
 * dày hơn: 500K → ~3/tuần · 200K → ~1/ngày · 100K → ~2/ngày.
 *
 * Đọc đè bằng BAO_CAO_TUAN_NGUONG_VIEW để đổi ngưỡng mà không phải sửa code.
 */
 ...
 */
export function filterByThreshold<T extends { view_count: number }>(videos: T[], nguong: number) {
  const dat: T[] = [];
  const belowThreshold: T[] = [];
  for (const v of videos) (v.view_count >= nguong ? dat : belowThreshold).push(v);
  return { dat, belowThreshold };
}

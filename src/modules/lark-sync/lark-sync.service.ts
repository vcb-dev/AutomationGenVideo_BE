...
export class LarkSyncService implements OnApplicationBootstrap {
  ...
  private readonly legacySyncEnabled =
    String(
      process.env.LARK_ENABLE_LEGACY_SYNC_SERVICE ?? "false",
    ).toLowerCase() === "true";
  ...
  private shouldWriteDirectToServer(): boolean {
    const flag = String(
      process.env.LARK_SYNC_DIRECT_TO_SERVER ?? "false",
    ).toLowerCase();
  ...
  async onApplicationBootstrap() {
    ...
    this.logger.log(
      `🚀 Server started — deferring Lark master sync by ${DELAY_MS / 1000}s...`,
    );
  ...
  private async runBootstrapSync() {
    this.logger.log(
      "🔄 Starting deferred bootstrap (Unified Sync + Đồ Da + Permissions)...",
    );
    ...
    try {
      ...
      this.logger.error(
        `❌ Bootstrap Unified Sync failed: ${err?.message ?? err}`,
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `✅ Bootstrap KPI Đồ Da sync: ${kpiDd?.synced ?? 0} records`,
      );
    } catch (err: any) {
      this.logger.error(
        `❌ Bootstrap KPI Đồ Da sync failed: ${err?.message ?? err}`,
      );
    }
    ...
    try {
      ...
      this.logger.error(
        `❌ Bootstrap Permission sync failed: ${err?.message ?? err}`,
      );
    }
    try {
      const doda = await this.larkService.syncDoDaChannelData();
      this.logger.log(
        `✅ Bootstrap Do Da channel sync: ${doda?.synced ?? 0} records`,
      );
    } catch (err: any) {
      this.logger.error(
        `❌ Bootstrap Do Da channel sync failed: ${err?.message ?? err}`,
      );
    }
  ...
  async unifiedSync() {
    ...
    const run = async (name: string, fn: () => Promise<any>) => {
      try {
        ...
        this.logger.log(
          `✅ ${name}: ${result?.synced ?? result?.count ?? "done"}`,
        );
      } catch (err: any) {
    ...
    try {
      if (isChannelSyncOn) {
        ...
        this.logger.log(
          "[MasterSync] Channel sync skipped (LARK_ENABLE_CHANNEL_SYNC=false)",
        );
      }
      ...
      if (isChannelSyncOn) {
        ...
        this.logger.log(
          "[MasterSync] DoDa channel sync skipped (LARK_ENABLE_CHANNEL_SYNC=false)",
        );
      }
  ...
  @Cron("0 10 */3 * * *", {
    name: "sync-all-fast-script",
    timeZone: "Asia/Ho_Chi_Minh",
  })
  async runSyncAllFastScript() {
    const enabled = String(
      process.env.LARK_ENABLE_SYNC_ALL_FAST_CRON ?? "true",
    ).toLowerCase();
    if (enabled === "0" || enabled === "false" || enabled === "no") {
      this.logger.log(
        "[sync:all:fast cron] disabled by LARK_ENABLE_SYNC_ALL_FAST_CRON",
      );
      return;
    }
    ...
    if (this.syncLock) {
      this.logger.warn(
        "[sync:all:fast cron] skipped — another sync is in progress",
      );
      return;
    }
    ...
    try {
      ...
      if (error?.stdout)
        this.logger.error(`[sync:all:fast cron][stdout]\n${error.stdout}`);
      if (error?.stderr)
        this.logger.error(`[sync:all:fast cron][stderr]\n${error.stderr}`);
    } finally {
      this.syncLock = false;
    }
  }
  ...
  async getSyncStatus(): Promise<{
    ...
    const dbUsers = await this.getUserPrisma().findMany({
      select: { email: true },
    });

...
function normalizeLarkChannelActivityStatus(
...
function isLarkChannelActiveStatus(status: string | null | undefined): boolean {
  ...
  return (
    s === "đang hoạt động" || s === "on" || s === "active" || s === "hoạt động"
  );
}

/** Chuẩn hóa tên platform từ Lark về dạng chuẩn dùng trong DB/FE */
function normalizePlatformName(raw: string | null | undefined): string {
  if (!raw) return "";
...
/** Chuẩn hóa giá trị status từ Lark về tiếng Việt lowercase để FE map được */
function normalizeLarkStatusToVietnamese(
  raw: string | null | undefined,
): string {
  if (!raw) return "đang hoạt động";
  const lower = raw.toLowerCase().trim().replace(/\s+/g, " ").normalize("NFC");
  if (
    lower === "on" ||
    lower === "active" ||
    lower === "hoạt động" ||
    lower === "đang hoạt động"
  )
    return "đang hoạt động";
  if (
    lower === "off" ||
    lower === "ngừng hoạt động" ||
    lower === "ngung hoat dong"
  )
  ...
  implements OnModuleInit, OnApplicationBootstrap, OnModuleDestroy
{
  ...
  constructor(
    ...
    this.REPORT_TABLE_ID = this.configService.get<string>(
      "LARK_REPORT_TABLE_ID",
    );
    ...
    this.EMPLOYEE_TABLE_ID = this.configService.get<string>(
      "LARK_EMPLOYEE_TABLE_ID",
    );
    this.PERMISSION_TABLE_ID = this.configService.get<string>(
      "LARK_PERMISSION_TABLE_ID",
    );
    this.LIST_TASK_TABLE_ID =
      this.configService.get<string>("LARK_LIST_TASK_TABLE_ID") ||
      "tblUubDhUoJ9TV7m";
    this.OUTSTANDING_TABLE_ID =
      this.configService.get<string>("LARK_OUTSTANDING_TABLE_ID") ||
      "tbluurIuf2qDCdFr";
    this.TRAFFIC_TABLE_ID =
      this.configService.get<string>("LARK_TRAFFIC_TABLE_ID") ||
      "tblsybBYaPKfsqQK";
    this.KPI_DODA_BASE_ID =
      this.configService.get<string>("LARK_KPI_DODA_BASE_ID") ||
      "Livew1AE0i2vo5kF3YXlCPNWg8f";
    this.KPI_DODA_TABLE_ID =
      this.configService.get<string>("LARK_KPI_DODA_TABLE_ID") ||
      "tblI1NzUOszaehhQ";
    // Keep activity data cached longer to cut repeated SQL load from frequent polling.
    this.activitySharedCacheTtlMs = Math.max(
      30_000,
      Number(
        this.configService.get<string>("LARK_ACTIVITY_SHARED_CACHE_TTL_MS") ||
          5 * 60 * 1000,
      ),
    );
    this.activityRoleCacheTtlMs = Math.max(
      60_000,
      Number(
        this.configService.get<string>("LARK_ACTIVITY_ROLE_CACHE_TTL_MS") ||
          30 * 60 * 1000,
      ),
    );
  }
  ...
  async onModuleInit() {
    this.logger.log(
      "LarkService initialized, invalidating activity reports cache...",
    );
  ...
  async onApplicationBootstrap() {
    ...
    this.logger.log(
      `🚀 [Bootstrap] Server started — sẽ tự động sync Lark sau ${DELAY_MS / 1000}s...`,
    );
    setTimeout(() => {
      this.logger.log(
        "🔄 [Bootstrap] Kích hoạt tiến trình Sync dữ liệu Lark (Non-blocking)...",
      );
      this.handleCron().catch((err) =>
        this.logger.error("CRON Bootstrap failure", err),
      );
    }, DELAY_MS);
  }
  ...
  private get prismaLarkKpiDoDa(): Prisma.LarkKPIDelegate {
    return (this.prisma as unknown as { larkKpiDoDa: Prisma.LarkKPIDelegate })
      .larkKpiDoDa;
  }

  /** Bảng KPI Đồ Da tách riêng theo Người edit + Ngày edit. */
  private get prismaLarkKpiDoDaEditor() {
    return (this.prisma as unknown as { larkKpiDoDaEditor: any })
      .larkKpiDoDaEditor;
  }
  ...
  private createRemotePrismaClient(url: string): PrismaClient {
    ...
    if (!client) {
      client = new PrismaClient({
        datasources: { db: { url: buildScopedPrismaDbUrl(url) } },
      });
      ...
      this.logger.log(
        `Created and cached remote PrismaClient for URL: ${url.replace(/:[^:@]+@/, ":***@")}`,
      );
    }
    return client;
  }
    ...
    for (const [url, client] of this.remotePrismaClients.entries()) {
      ...
      if (originalDisconnect) {
        await originalDisconnect().catch((e: any) =>
          this.logger.warn(
            `Failed to disconnect cached remote PrismaClient: ${e.message}`,
          ),
        );
      }
    }
  ...
  private getKpiMinDateKey(): string {
    const envVal = String(
      this.configService.get<string>("LARK_KPI_MIN_DATE") || "",
    ).trim();
  ...
  private getKpiDoDaMinDateKey(): string {
    const envVal = String(
      this.configService.get<string>("LARK_KPI_DODA_MIN_DATE") || "",
    ).trim();
    if (/^\d{4}-\d{2}-\d{2}$/.test(envVal)) return envVal;
    const nowVN = new Intl.DateTimeFormat("en-CA", {
      timeZone: "Asia/Ho_Chi_Minh",
  ...
  getDbTargetsDebugInfo() {
    ...
    return {
      ...
      kpiMirrorEnabled: String(
        this.configService.get<string>("LARK_KPI_MIRROR_TO_SERVER") ?? "true",
      ),
      channelMirrorEnabled: String(
        this.configService.get<string>("LARK_CHANNEL_MIRROR_TO_SERVER") ??
          "true",
      ),
  ...
  private getDirectSyncDbUrl(): string | null {
    const flag = String(
      this.configService.get<string>("LARK_SYNC_DIRECT_TO_SERVER") ?? "false",
    ).toLowerCase();
    if (flag === "0" || flag === "false" || flag === "no") return null;
    const serverRaw = this.configService
      .get<string>("SERVER_DATABASE_URL")
      ?.trim();
    if (!serverRaw) return null;
    const su = this.stripEnvQuotes(serverRaw);
    const lu = this.stripEnvQuotes(
      String(this.configService.get<string>("DATABASE_URL") || "").trim(),
    );
    if (lu && su === lu) return null;
    return su;
  }
  ...
  private getMirrorRetryCount(): number {
    const n = Number(
      this.configService.get<string>("LARK_KPI_MIRROR_RETRIES") || "3",
    );
  ...
  private shouldRunBackgroundChannelEnrich(): boolean {
    const flag = String(
      this.configService.get<string>("LARK_ENABLE_BACKGROUND_CHANNEL_ENRICH") ??
        "false",
    ).toLowerCase();
  ...
  private getBackgroundChannelEnrichConcurrency(): number {
    const n = Number(
      this.configService.get<string>(
        "LARK_BACKGROUND_CHANNEL_ENRICH_CONCURRENCY",
      ) || "1",
    );
  ...
  private async mirrorLarkKpiSnapshotToServer(
    ...
    try {
      ...
      this.logger.log(
        `[KPI] Mirrored ${rows.length} lark_kpi row(s) to remote DB (replace-all).`,
      );
      return rows.length;
    } finally {
  ...
  private async mirrorLarkKpiDoDaSnapshotToServer(
    rows: Record<string, unknown>[],
  ): Promise<number> {
    const url = this.getRemoteMirrorDbUrl();
    if (!url) return 0;
    const remote = this.createRemotePrismaClient(url);
    const delegate = (
      remote as unknown as { larkKpiDoDa: Prisma.LarkKPIDelegate | undefined }
    ).larkKpiDoDa;
    if (!delegate) {
      await remote.$disconnect().catch(() => undefined);
      this.logger.warn(
        "[KPI DoDa] Remote Prisma has no larkKpiDoDa delegate — skip mirror.",
      );
      return 0;
    }
    const maxRetries = this.getMirrorRetryCount();
    ...
    try {
      ...
      this.logger.log(
        `[KPI DoDa] Mirrored ${rows.length} lark_kpi_do_da row(s) to remote DB.`,
      );
      return rows.length;
    } finally {
      await remote.$disconnect().catch(() => undefined);
  ...
  private async mirrorLarkKpiDoDaEditorSnapshotToServer(
    rows: Record<string, unknown>[],
  ): Promise<number> {
    const url = this.getRemoteMirrorDbUrl();
    if (!url) return 0;
    const remote = this.createRemotePrismaClient(url);
    const delegate = (remote as unknown as { larkKpiDoDaEditor: any })
      .larkKpiDoDaEditor;
    if (!delegate) {
      await remote.$disconnect().catch(() => undefined);
      this.logger.warn(
        "[KPI DoDa Editor] Remote Prisma has no larkKpiDoDaEditor delegate — skip mirror.",
      );
      return 0;
    }
    const maxRetries = this.getMirrorRetryCount();
    ...
    try {
      ...
      this.logger.log(
        `[KPI DoDa Editor] Mirrored ${rows.length} row(s) to remote DB.`,
      );
      return rows.length;
    } finally {
      await remote.$disconnect().catch(() => undefined);
  ...
  private shouldMirrorChannels(): boolean {
    const flag = String(
      this.configService.get<string>("LARK_CHANNEL_MIRROR_TO_SERVER") ?? "true",
    ).toLowerCase();
    if (flag === "0" || flag === "false" || flag === "no") return false;
    return true;
  }

  private async mirrorChannelSnapshotToServer(
    rows: Record<string, unknown>[],
  ): Promise<number> {
    const url = this.getRemoteMirrorDbUrl();
    ...
    try {
      await this.withRetry(
        async () => {
          await remote.channel.deleteMany({
            where: { NOT: { id: { startsWith: "doda_" } } },
          });
          for (let i = 0; i < rows.length; i += CHUNK) {
            const chunk = rows.slice(i, i + CHUNK);
            if (chunk.length)
              await remote.channel.createMany({
                data: chunk as any,
                skipDuplicates: true,
              });
          }
      ...
      this.logger.log(
        `[Channel] Mirrored ${rows.length} channel row(s) to remote DB (kept Do Da).`,
      );
      return rows.length;
    } finally {
      await remote.$disconnect().catch(() => undefined);
  ...
  private async mirrorDoDaChannelSnapshotToServer(
    rows: Record<string, unknown>[],
  ): Promise<number> {
    const url = this.getRemoteMirrorDbUrl();
    if (!url || !this.shouldMirrorChannels()) return 0;
    ...
    try {
      ...
      this.logger.log(
        `[Channel DoDa] Mirrored ${rows.length} row(s) to remote DB.`,
      );
      return rows.length;
    } finally {
      await remote.$disconnect().catch(() => undefined);
  ...
  private async syncLocalSnapshotToServer(): Promise<{
    ...
    if (!url) {
      this.logger.log(
        "[Local->Server] Skipped: SERVER_DATABASE_URL is not configured (or equals DATABASE_URL).",
      );
    ...
    const remote = this.createRemotePrismaClient(url);
    const remoteDoDa = (
      remote as unknown as { larkKpiDoDa?: Prisma.LarkKPIDelegate }
    ).larkKpiDoDa;
    const remoteDoDaEditor = (remote as unknown as { larkKpiDoDaEditor?: any })
      .larkKpiDoDaEditor;
    ...
    try {
      ...
      const [kpis, kpiDoDaRows, kpiDoDaEditorRows] = await Promise.all([
        ...
        this.prismaLarkKpiDoDa.findMany({
          where: { report_date: kpiDateWindow },
        }),
      ...
      if (remoteDoDa) {
        ...
        this.logger.warn(
          "[Local->Server] Remote Prisma has no larkKpiDoDa delegate - skipped lark_kpi_do_da.",
        );
      }
      ...
      if (remoteDoDaEditor) {
          ...
          async () => {
            try {
              ...
              for (let i = 0; i < kpiDoDaEditorRows.length; i += CHUNK) {
                ...
                if (chunk.length)
                  await remoteDoDaEditor.createMany({
                    data: chunk as any,
                    skipDuplicates: true,
                  });
              }
              ...
              if (
                msg.includes("lark_kpi_do_da_editor") &&
                msg.toLowerCase().includes("does not exist")
              ) {
                this.logger.warn(
                  "[Local->Server] Remote DB missing table lark_kpi_do_da_editor - skipped mirror.",
                );
                return;
              }
              throw err;
        ...
        this.logger.warn(
          "[Local->Server] Remote Prisma has no larkKpiDoDaEditor delegate - skipped lark_kpi_do_da_editor.",
        );
      }
  ...
  async pullKpiFromServer(): Promise<{ pulled: number }> {
    ...
    if (!url) {
      this.logger.warn(
        "[Server->Local] Skipped: SERVER_DATABASE_URL kh�ng được cấu h�nh (hoặc tr�ng DATABASE_URL).",
      );
      return { pulled: 0 };
    }
    ...
    try {
      ...
      this.logger.log(
        `[Server->Local] Lấy ${rows.length} lark_kpi row(s) từ server...`,
      );
      ...
      this.logger.log(
        `[Server->Local] Đ� pull ${rows.length} lark_kpi row(s) v�o local DB.`,
      );
      this.invalidateActivityCache();
  ...
  async getAccessToken(): Promise<string> {
    if (!this.APP_ID || !this.APP_SECRET) {
      throw new Error(
        "LARK_APP_ID and LARK_APP_SECRET must be configured in .env file",
      );
    }
  ...
  async handleCron() {
    this.logger.log(
      "[Cron] Start full Lark sync: Lark -> local + Lark -> server",
    );
    ...
    const channelSyncEnabled = String(
      process.env.LARK_ENABLE_CHANNEL_SYNC ?? "true",
    ).toLowerCase();
    if (
      ...
      this.logger.log(
        "[Cron] Channel sync skipped (LARK_ENABLE_CHANNEL_SYNC=false)",
      );
    }

    this.logger.log(
      `[Cron] Finished full sync flow in ${Date.now() - startTime}ms.`,
    );
  }
  @Cron("0 10 8,10,12,14,16,18,20,22 * * *", {
    name: "lark-activity-cache-flush",
    timeZone: "Asia/Ho_Chi_Minh",
  })
  ...
  @Cron("0 20 12 * * *", {
    name: "lark-data-cleanup",
    timeZone: "Asia/Ho_Chi_Minh",
  }) // cleanup chỉ cần 1 lần/ngày
  async handleCleanup() {
    this.logger.log(
      "Starting scheduled data cleanup for Lark tables (daily at 12:20)...",
    );
    try {
      ...
      const reportResult = await this.prisma.larkReport.deleteMany({
        where: {
          OR: [{ name: { equals: "Unknown" } }, { name: { equals: "" } }],
        },
      });
  ...
  async syncReportData() {
    try {
      const records = await this.fetchLarkRecordsGeneric(
        this.REPORT_BASE_ID,
        this.REPORT_TABLE_ID,
        500,
      );
      this.logger.log(
        `Fetched ${records.length} Report records from Lark (Table: ${this.REPORT_TABLE_ID}). Syncing to database...`,
      );
  ...
  async syncTrafficData() {
    this.logger.log(
      "[LarkSync] syncTrafficData disabled - this table is now independent.",
    );
    return;
  }
  ...
  async uploadEvidenceToLark(
    ...
    try {
      ...
      this.logger.error(
        "Failed to upload evidence to Lark:",
  ...
  async submitTrafficReport(payload: any) {
    ...
    if (duplicateOr.length > 0) {
      ...
      if (existingTraffic) {
        ...
        if (
          payloadTeam &&
          String(payloadTeam).trim() &&
          existingTraffic.team !== String(payloadTeam).trim()
        ) {
          // Different team — allow submission to continue
        } else {
          return {
            message:
              "Bạn đã báo cáo traffic cho ngày này rồi. Hệ thống giữ dữ liệu đã báo cáo.",
            alreadySubmitted: true,
            existingRecordDate:
              existingTraffic.created_at || existingTraffic.date,
            recordIds: [],
          };
        }
      }
    ...
    if (payloadTeam && String(payloadTeam).trim()) {
      ...
      const userRecord = await this.prisma.user.findFirst({
        where: {
          email: {
            equals: normalizedSubmitterEmail,
      ...
      platformEntries.forEach((entry: any) => {
        ...
        if (val > 0) {
          ...
          if (entry.evidences && entry.evidences.length > 0) {
            data[`evidence_${pKey}`] = JSON.stringify(
              entry.evidences.map((ev: any) => ev.token),
            );
          }
  ...
  async submitChecklistReport(payload: any) {
    ...
    const { email, name, team, reportDate, userEmail, userName, userTeam } =
      payload;
    ...
    const finalName = String(
      name || userName || userRec?.full_name || "Unknown",
    );
    ...
    this.logger.debug(
      `[submitChecklistReport] Payload: ${JSON.stringify({ email, name, team, userEmail, userName, userTeam })}`,
    );
    this.logger.debug(
      `[submitChecklistReport] finalName: "${finalName}", finalTeam: "${finalTeam}"`,
    );
    ...
    const existing = await this.prisma.larkReport.findFirst({
      where: {
        date: { gte: bounds.start, lte: bounds.end },
        team: finalTeam
          ? { equals: finalTeam, mode: "insensitive" as any }
          : undefined,
        OR: [
    ...
    const reportId =
      existing?.id ||
      `local_chk_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
  ...
  async syncOutstandingData() {
    this.logger.log(
      "[LarkSync] syncOutstandingData disabled - this table is now independent.",
    );
    return;
  }
  ...
  async getPermissionData() {
    ...
    return users.map((u) => ({
      ...
      employee: u.employee_id
        ? JSON.stringify([
            { id: u.employee_id, name: u.full_name, avatar_url: u.image_url },
          ])
        : null,
  ...
  async syncChannelData() {
    try {
      const baseId =
        this.configService.get<string>("LARK_CHANNEL_BASE_ID") ||
        "JAEmwmWQkixHOOkumU5lRU7ogkb";
      const tableId =
        this.configService.get<string>("LARK_CHANNEL_TABLE_ID") ||
        "tblWxMtDAkvh1gWS";
      ...
      this.logger.log(
        `Fetched ${records.length} records from Channel table. Overwriting Channel model...`,
      );
      ...
      // Pre-load users để resolve owner_id từ tên/email ngay trong vòng lặp.
      const allUsers = await this.prisma.user.findMany({
        where: { is_active: true },
        select: { id: true, email: true, full_name: true },
      });
      // Map: exact normalized name → user_id
      const exactNameToUserId = new Map<string, string>();
      // Map: sorted-words → user_id (chỉ dùng khi key là DUY NHẤT)
      const sortedKeyCount = new Map<string, number>();
      const sortedToUserId = new Map<string, string>();
      // Map: email (lowercase) → user_id
      const emailToUserId = new Map<string, string>();
      for (const u of allUsers) {
        if (u.email) emailToUserId.set(u.email.toLowerCase(), u.id);
        if (!u.full_name) continue;
        const norm = this.normalizeOwnerName(u.full_name);
        if (!exactNameToUserId.has(norm)) exactNameToUserId.set(norm, u.id);
        const sorted = norm.split(" ").sort().join(" ");
        if (sorted !== norm) {
          sortedKeyCount.set(sorted, (sortedKeyCount.get(sorted) ?? 0) + 1);
          sortedToUserId.set(sorted, u.id);
        }
      }
      // Pre-load teams để resolve team_id từ tên team.
      const allTeams = await this.prisma.team.findMany({
        select: { id: true, name: true },
      });
      const teamNameToId = new Map<string, string>(
        allTeams.map((t) => [t.name.toLowerCase().trim(), t.id]),
      );
      const resolveOwnerIdFromName = (
        ownerName: string,
        ownerEmail?: string | null,
      ): string | null => {
        if (ownerEmail) {
          const byEmail = emailToUserId.get(ownerEmail.toLowerCase());
          if (byEmail) return byEmail;
        }
        if (!ownerName) return null;
        const norm = this.normalizeOwnerName(ownerName);
        if (exactNameToUserId.has(norm)) return exactNameToUserId.get(norm)!;
        const sorted = norm.split(" ").sort().join(" ");
        if (
          (sortedKeyCount.get(sorted) ?? 0) === 1 &&
          sortedToUserId.has(sorted)
        ) {
          return sortedToUserId.get(sorted)!;
        }
        return null;
      };
      ...
      for (const record of records) {
        ...
        const name =
          extractString(f["Tên kênh hiện tại"]) ||
          extractString(f["Tên kênh A?"]) ||
          extractString(f["name"]) ||
          "N/A";

        const owner =
          extractString(f["Nhân viên traffic xây kênh"]) ||
          extractString(f["NV traffic xây kênh"]) ||
          extractString(f["owner A?"]) ||
          "";

        const ownerEmailRaw =
          extractEmail(f["Nhân viên traffic xây kênh"]) ||
          extractEmail(f["NV traffic xây kênh"]);
        const rawPlatform =
          extractString(f["Nền tảng"]) || extractString(f["Nền tảng A?"]) || "";
        const rawStatus =
          extractString(
        ...
        const data = {
          ...
          channel_id:
            extractString(f["ID kênh hiện tại"]) ||
          ...
          link_channel:
            ...
            "",
          status: normalizeLarkStatusToVietnamese(rawStatus),
          team_id: teamNameToId.get(teamTraffic?.toLowerCase().trim()) ?? null,
          owner_id: resolveOwnerIdFromName(owner, ownerEmailRaw),
        };
      ...
      if (channelsToInsert.length > 0) {
        this.logger.log(
          `Syncing ${channelsToInsert.length} fresh records to Channel atomically...`,
        );
      ...
      this.logger.log(
        `Successfully synced ${synced}/${records.length} records to Channel (skipped ${skippedTeam} from excluded teams).`,
      );

      // Second-pass: resolve owner_id cho các channel chưa match được trong vòng lặp chính
      try {
        const enriched = await this.enrichChannelOwnerIds();
        this.logger.log(
          `[Channel] owner_id enrichment: updated ${enriched} channels.`,
        );
      } catch (enrichErr: any) {
        this.logger.warn(
          `[Channel] owner_id enrichment failed: ${enrichErr?.message}`,
        );
      }
      ...
      try {
        ...
        this.logger.warn(
          `[Lark] import tracked after channel sync: ${ie?.message}`,
        );
      }
      ...
      try {
        ...
        const mirroredRemote = await this.mirrorChannelSnapshotToServer(
          localRows as any,
        );
        if (mirroredRemote)
          this.logger.log(
            `[Channel] Remote mirror success: ${mirroredRemote} row(s).`,
          );
      } catch (mirrorErr: any) {
        this.logger.error(
          "[Channel] Mirror to remote DB failed — local channel is already updated",
          mirrorErr,
        );
      }
    } catch (error) {
  ...
  /**
   * Second-pass: owner_id đã được resolve inline trong syncChannelData/syncDoDaChannelData.
   * Hàm này giữ lại cho backward-compat với các caller cũ.
   */
  async enrichChannelOwnerIds(): Promise<number> {
    return 0;
  }

  /**
   * Sync kênh team Đồ Da từ Lark Bitable riêng vào Channel table.
   * team_id resolve từ tên "Đồ Da", owner_id resolve từ tên/email nhân viên.
   */
  async syncDoDaChannelData() {
    ...
    try {
      this.logger.log(
        `[DoDa] Syncing channels from base: ${DODA_BASE_ID}, table: ${DODA_TABLE_ID}`,
      );
      const records = await this.fetchLarkRecordsGeneric(
        DODA_BASE_ID,
        DODA_TABLE_ID,
      ...
      // Pre-load team + users để resolve FKs
      const dodaTeam = await this.prisma.team.findFirst({
        where: { name: { equals: TEAM_NAME, mode: "insensitive" } },
        select: { id: true },
      });
      const dodaTeamId = dodaTeam?.id ?? null;

      const dodaUsers = await this.prisma.user.findMany({
        where: { is_active: true },
        select: { id: true, email: true, full_name: true },
      });
      const dodaEmailToUserId = new Map<string, string>(
        dodaUsers
          .filter((u) => u.email)
          .map((u) => [u.email!.toLowerCase(), u.id]),
      );
      const dodaNameToUserId = new Map<string, string>();
      for (const u of dodaUsers) {
        if (u.full_name) {
          const norm = this.normalizeOwnerName(u.full_name);
          if (!dodaNameToUserId.has(norm)) dodaNameToUserId.set(norm, u.id);
        }
      }
      ...
      const extractString = (val: any): string | null => {
        ...
        if (Array.isArray(val)) {
          ...
          if (typeof first === "object" && first !== null) {
            return (
              first.text || first.name || first.value || first.en_name || null
            );
          }
          return String(first);
        }
      ...
      for (const record of records) {
        ...
        const name =
          extractString(f["Tên Kênh"]) || extractString(f["Tên kênh"]) || "N/A";
        ...
        if (Array.isArray(larkAccount) && larkAccount.length > 0) {
          larkEmail = larkAccount[0].email || null;
        }

        // Resolve owner_id: ưu tiên email từ Lark account, fallback tên
        const ownerIdDoda =
          (larkEmail && dodaEmailToUserId.get(larkEmail.toLowerCase())) ||
          (owner && dodaNameToUserId.get(this.normalizeOwnerName(owner))) ||
          null;

        const platformRaw =
          extractString(f["Nền Tảng"]) || extractString(f["Nền tảng"]) || "";
        ...
        const data = {
          ...
          channel_id: "",
          link_channel:
            extractUrl(f["Link kênh"]) || extractUrl(f["Link Kênh"]) || "",
          status: normalizeLarkStatusToVietnamese(dodaStatusRaw),
          team_id: dodaTeamId,
          owner_id: ownerIdDoda,
        };
        ...
        try {
          ...
          this.logger.warn(
            `[DoDa] Skip record ${record.record_id}: ${e?.message}`,
          );
        }
      }
      ...
      // owner_id đã được resolve inline ở trên, không cần enrichment riêng

      // Import vào tracked_channels
      try {
        const imp = await this.importTrackedChannelsFromChannelTable();
        this.logger.log(
          `[DoDa] tracked_channels import: imported=${imp.imported}`,
        );
      } catch (ie: any) {
      ...
      try {
        const dodaRows = await this.prisma.channel.findMany({
          where: { id: { startsWith: "doda_" } },
        });
        const mirroredRemote = await this.mirrorDoDaChannelSnapshotToServer(
          dodaRows as any,
        );
        if (mirroredRemote)
          this.logger.log(
            `[Channel DoDa] Remote mirror success: ${mirroredRemote} row(s).`,
          );
      } catch (mirrorErr: any) {
        this.logger.error(
          "[Channel DoDa] Mirror to remote DB failed — local channel is already updated",
          mirrorErr,
        );
      }
  ...
  async importTrackedChannelsFromChannelTable(opts?: {
    ...
    // Lấy tất cả rows có status không rỗng, kèm owner relation để resolve user trực tiếp
    const baseWhere: any = {
      AND: [
        { status: { not: null } },
        { NOT: { status: "" } },
        // Nếu chỉ sync cho 1 user cụ thể, filter trực tiếp qua FK
        ...(opts?.onlyUserId ? [{ owner_id: opts.onlyUserId }] : []),
      ],
    };
    let rows = await this.prisma.channel.findMany({
      where: baseWhere,
      include: {
    ...
    for (const row of rows) {
      try {
        ...
        // Với FK owner_id, user lookup là trực tiếp qua relation
        const user = row.owner ?? null;
        if (!user) {
        ...
        const hasNoData =
          !existingTc ||
          ((existingTc.total_followers == null ||
            existingTc.total_followers === 0) &&
            Number(existingTc.total_likes) === 0 &&
            (existingTc.total_videos == null || existingTc.total_videos === 0));
        const isStaleWithNoData =
          hasNoData &&
          existingTc?.last_synced_at != null &&
          Date.now() - new Date(existingTc.last_synced_at).getTime() >
            STALE_THRESHOLD_MS;
    ...
    if (enrichQueue.length > 0) {
      if (!this.shouldRunBackgroundChannelEnrich()) {
        ...
        this.channelStatsEnrichment
          .enrichBatch(enrichQueue, {
            concurrency: this.getBackgroundChannelEnrichConcurrency(),
          })
          .then(({ ok, failed }) => {
            this.logger.log(
              `[Lark] Làm giàu số liệu xong: thành công=${ok}, lỗi/bỏ qua=${failed}`,
            );
          })
          .catch((e) => {
            this.logger.warn(
              `[Lark] Làm giàu số liệu hàng loạt lỗi (background): ${e?.message || e}`,
            );
          });
      }
    }

    const allCh = await this.prisma.channel.findMany({
      select: { id: true, status: true },
      take: 5000,
    });
    const inactiveLarkIds = allCh
      .filter((ch) => !isLarkChannelActiveStatus(ch.status))
      .map((ch) => ch.id);
  ...
  async importTrackedChannelsForUser(
    userId: string,
    prioritizePlatform?: string,
  ) {
    return this.importTrackedChannelsFromChannelTable({
      onlyUserId: userId,
      prioritizePlatform,
    });
  }
  ...
  async getChannelData(owner?: string, team?: string, email?: string) {
    ...
    // Resolve owner_id từ email hoặc tên (giao diện cũ truyền owner name / email)
    if (email || owner) {
        ...
        where: {
          OR: [
            ...(email
              ...
              : []),
            ...(owner
              ? [{ full_name: { equals: owner, mode: "insensitive" as const } }]
              : []),
          ],
        },
        select: { id: true },
      ...
      if (ownerUser) {
        andConditions.push({ owner_id: ownerUser.id });
      }
    }

    // Optional team filter qua nested relation
    if (team) {
      andConditions.push({
        team: { name: { contains: team, mode: "insensitive" } },
      });
    }
  ...
  async fetchLarkRecordsGeneric(
    ...
    try {
      while (hasMore) {
        ...
        const token = await this.getAccessToken();
        this.logger.debug(
          `Fetching: ${url} (Token: ${token.substring(0, 10)}...)`,
        );
  ...
  async inspectTableStructure() {
    try {
      ...
      if (records.length > 0) {
        ...
        const recordsWithAnswers = records.filter(
          (r) => r.fields["Answers"] || r.fields["Date"],
        );
        this.logger.log(
        ...
        if (recordsWithAnswers.length > 0) {
          ...
          for (let i = 0; i < Math.min(3, recordsWithAnswers.length); i++) {
            this.logger.log(`\nRecord ${i + 1}:`);
            this.logger.log(
              JSON.stringify(recordsWithAnswers[i].fields, null, 2),
            );
          }
        }
        ...
        return {
          ...
          sampleRecordsWithData: recordsWithAnswers
            .slice(0, 3)
            .map((r) => r.fields),
  ...
  private mapRecordToReport(record: any, dbUsersMap: Map<string, any>) {
    ...
    const extractString = (val: any): string | null => {
      ...
      if (Array.isArray(val) && val.length > 0) {
        const first = val[0];
        return (
          first.name || first.text || (typeof first === "string" ? first : null)
        );
      }
    ...
    let name = extractString(
      fields["HoTen"] || fields["Họ tên"] || fields["Nhân viên"],
    );
  ...
  async syncEmployeeData() {
    try {
      ...
      this.logger.log(
        `Fetched ${records.length} employee records from Lark. Syncing to database...`,
      );
      ...
      if (records.length > 0) {
        ...
        Object.keys(firstRecord.fields).forEach((key) => {
          this.logger.debug(
            `Field: "${key}" = ${JSON.stringify(firstRecord.fields[key])}`,
          );
        });
      }
      ...
      for (const record of records) {
        ...
        if (!employeeData.name || employeeData.name === "Unknown") {
          this.logger.debug(
            `Skipping garbage employee record: ${record.record_id}`,
          );
          continue;
        }

        const syntheticEmail = `lark-${String(employeeData.id).replace(/[^a-zA-Z0-9]/g, "_")}@employee.vcb.internal`;
        ...
        const byEmail = employeeData.email
          ? await this.prisma.user.findFirst({
              where: {
                email: {
                  equals: employeeData.email.trim(),
                  mode: "insensitive",
                },
                NOT: { email: { endsWith: "@employee.vcb.internal" } },
              },
        ...
        if (!target) {
          this.logger.debug(
            `Skipping synthetic email creation for employee: ${employeeData.name}`,
          );
          syncedCount++;
          continue;
        }
        ...
        const empIdConflict =
          employeeData.employee_id && byEmpId && byEmpId.id !== target.id;
        ...
        const payload = {
          ...
          team:
            this.mergeTeamValues(target.team, employeeData.team) ?? undefined,
  ...
  private mapRecordToEmployee(record: any) {
    ...
    let name = extractString(
      fields["Tên"] || fields["Ten"] || fields["Họ tên"] || fields["Nhân viên"],
    );
    ...
    if (
      fields["Hình ảnh"] &&
      Array.isArray(fields["Hình ảnh"]) &&
      fields["Hình ảnh"].length > 0
    ) {
      imageUrl =
        fields["Hình ảnh"][0].url || fields["Hình ảnh"][0].tmp_url || null;
    }

    // report_date: Prefer 'Ngày báo cáo', then 'Ngày', then 'NGÀY'
    let reportDate = null;
    const rawReportDate =
      fields["Ngày báo cáo"] || fields["Ngày"] || fields["Ngay"];
    if (rawReportDate) {
    ...
    return {
      id: record.record_id,
      employee_id: fields["ID nhân viên"]
        ? String(fields["ID nhân viên"]).trim()
        : null,
      name: name || "Unknown",
      email: email,
      image_url: imageUrl,
  ...
  async syncKPIData(options?: {
    forceLocalWrite?: boolean;
    skipRemoteMirror?: boolean;
  }) {
    // Only sync records within [March 1st of current year .. today] (Vietnam calendar day)
    const KPI_MIN_DATE_KEY = this.getKpiMinDateKey();
    const KPI_MAX_DATE_KEY = this.getKpiMaxDateKey();
    const directDbUrl = options?.forceLocalWrite
      ? null
      : this.getDirectSyncDbUrl();
    const targetClient = directDbUrl
      ? this.createRemotePrismaClient(directDbUrl)
      : null;
    const targetLarkKPI = targetClient
      ? targetClient.larkKPI
      : this.prisma.larkKPI;
    ...
    try {
      ...
      this.logger.log(
        `Fetched ${records.length} KPI records from Lark (Table: ${this.KPI_TABLE_ID}). Clearing database and syncing...`,
      );
      if (!records || records.length === 0) {
        throw new Error(
          "KPI sync aborted: no records fetched from Lark, skip replacing local table.",
        );
      }
      ...
      for (const record of records) {
        ...
        const recordDate =
          kpiData.report_date instanceof Date
            ? kpiData.report_date
        ...
        const nameKey = cleanName
          ...
          .trim()
          .replace(/\s+/g, " ");
        const empIdKey = kpiData.employee_id
          ? String(kpiData.employee_id).trim()
          : null;

        let larkUserId = null;
        if (kpiData.employee_data) {
          const empArr = Array.isArray(kpiData.employee_data)
            ? kpiData.employee_data
            : [kpiData.employee_data];
          if (empArr[0]?.id) larkUserId = String(empArr[0].id).trim();
        }

        const sysMatch =
          (empIdKey ? dbUsersMap.get(empIdKey) : null) ||
        ...
        if (sysMatch) {
          ...
          if (sysMatch.employee_status)
            kpiData.employee_status = sysMatch.employee_status;
        }
        ...
        if (
          sysMatch?.employee_status &&
          !isOnStatus(sysMatch.employee_status)
        ) {
          continue;
        }
      ...
      if (kpiRecordsToInsert.length > 0) {
        const CHUNK = 2000;
        this.logger.log(
          `[KPI] Transactionally replacing ${kpiRecordsToInsert.length} records...`,
        );
      ...
      if (!targetClient && !options?.skipRemoteMirror) {
        try {
          mirroredRemote =
            await this.mirrorLarkKpiSnapshotToServer(kpiRecordsToInsert);
        } catch (mirrorErr) {
          this.logger.error(
            "[KPI] Mirror to SERVER_DATABASE_URL failed — local lark_kpi is already updated",
            mirrorErr,
          );
        }
      }
      ...
      this.logger.log(
        `Successfully synced ${syncedCount} KPI records (skipped ${skippedOutsideDateWindow} records outside [${KPI_MIN_DATE_KEY} .. ${KPI_MAX_DATE_KEY}] or missing report_date).`,
      );
      this.invalidateActivityCache();
      return {
        synced: syncedCount,
  ...
  async syncKPIDoDaData(options?: {
    ...
    const targetDoDa = targetClient
      ? (
          targetClient as unknown as {
            larkKpiDoDa: Prisma.LarkKPIDelegate | undefined;
          }
        ).larkKpiDoDa
      : this.prismaLarkKpiDoDa;
    const targetDoDaEditor = targetClient
      ? (targetClient as unknown as { larkKpiDoDaEditor: any })
          .larkKpiDoDaEditor
      : this.prismaLarkKpiDoDaEditor;
    if (!this.KPI_DODA_BASE_ID || !this.KPI_DODA_TABLE_ID) {
      this.logger.warn(
        "[KPI DoDa] LARK_KPI_DODA_BASE_ID / LARK_KPI_DODA_TABLE_ID chưa cấu hình — bỏ qua sync.",
      );
      return {
        synced: 0,
        total: 0,
        skippedBeforeMinDate: 0,
        samples: [],
        allKeys: [],
      };
    }
    ...
    if (!targetDoDaEditor && isDoDaEditKpiTable) {
      throw new Error(
        "[KPI DoDa] Target Prisma has no larkKpiDoDaEditor delegate.",
      );
    }
    ...
    try {
      ...
      if (!records || records.length === 0) {
        throw new Error(
          "KPI DoDa sync aborted: no records fetched from Lark, skip replacing local table.",
        );
      }
      ...
      const isDoneByStatus = (statusField: any, checkedField: any): boolean => {
        ...
        if (
          checkedField === true ||
          checkedField === 1 ||
          String(checkedField).toLowerCase() === "true"
        )
          return true;
        if (Array.isArray(statusField)) {
          return statusField.some((x: any) => {
            const name = String(x?.name || x?.text || x || "")
              ...
              .trim();
            return (
              name.includes("da hoan thanh") || name.includes("hoan thanh")
            );
          });
        }
        return false;
      ...
      if (isDoDaEditKpiTable) {
        ...
        for (const record of records) {
          ...
          const rawDate =
            fields["Ngày edit"] || fields["Ngay edit"] || fields["Ngày đăng"];
          ...
          const editor = parseEditor(
            fields["Người edit"] || fields["Nguoi edit"],
          );
          const editorName = editor?.name ? String(editor.name).trim() : "";
          if (!editorName) continue;
          const nameKey = normalizeNameKey(editorName);
          const editorEmailKey = editor?.email
            ? String(editor.email).toLowerCase().trim()
            : null;
          const monthKey = dateKey.slice(0, 7);
          const done = isDoneByStatus(
            fields["Trạng thái"] || fields["Trang thai"],
            fields["Đã edit"] || fields["Da edit"],
          );

          const sysMatch =
            (editorEmailKey ? dbUsersMap.get(editorEmailKey) : null) ||
            dbUsersMap.get(nameKey);
          ...
          const monthAgg = monthTotals.get(monthTotalKey) || {
            total: 0,
            done: 0,
          };
          monthAgg.total += 1;
        ...
        dayBuckets.forEach((bucket, key) => {
          ...
          const monthAgg = monthTotals.get(
            `${nameBucketKey}|${teamBucketKey}|${bucket.monthKey}`,
          ) || { total: 0, done: 0 };
          editorRows.push({
            id: `doda-kpi-editor-${bucket.dateKey}-${nameBucketKey}-${teamBucketKey}`.slice(
              0,
              191,
            ),
      ...
      if (isDoDaEditKpiTable) {
        ...
        if (editorRows.length > 0) {
          ...
          for (let i = 0; i < editorRows.length; i += CHUNK) {
            ...
            await editorDelegate.createMany({
              data: chunk,
              skipDuplicates: true,
            });
          }
      ...
      if (!targetClient && !options?.skipRemoteMirror) {
        try {
          ...
          this.logger.error(
            "[KPI DoDa] Mirror to SERVER_DATABASE_URL failed — local DoDa KPI table is already updated",
            mirrorErr,
          );
        }
      }
  ...
  private mapRecordToKPI(record: any) {
        ...
        return val
          .map((item: any) => {
            if (item && typeof item === "object") {
              return (
                item.name || item.text || item.primary_val || item.title || ""
              );
            }
            return String(item);
          })
    ...
    if (!name) {
      name = extractString(
        findValue(["ID nhân viên", "Mã Nhân Viên VCB", "TAG"]),
      );
    }
    ...
    if (!name) {
      ...
      this.logger.warn(
        `KPI Record ${record.record_id} has no name. Fields: ${Object.keys(record.fields).join(", ")}. ${extraLog}`,
      );
    }
    ...
    return {
      id: record.record_id,
      employee_id:
        extractString(findValue(["ID nhân viên", "ID nhan vien"])) || null,
      name: name,
      ...
      completed_month: parseNum(
        findValue(["Hoàn thành Tháng", "Hoan thanh Thang"]),
      ),
      task_new: parseNum(findValue(["Task mới", "Task moi"])),
      task_new_month: parseNum(findValue(["Task mới tháng", "Task moi thang"])),
      task_auto: parseNum(
        findValue(["Số task tự động", "So task tu dong", "Task Auto"]),
      ),
      task_auto_month: parseNum(
        findValue(["Task Auto Tháng", "Task Auto Thang"]),
      ),
      task_creative: taskCreative,
      content_win_new: parseNum(
        findValue(["Content win mới", "Content win moi"]),
      ),
      revenue_month: cleanBigInt(
        findValue([
          "Doanh thu tháng",
          "Doanh thu thang",
          "Doanh thu",
      ...
      traffic_month: cleanBigInt(
        findValue(["Traffic Tháng", "Traffic Thang", "Traffic", "Traffi"]),
      ),
      target_revenue_month:
        extractString(
          findValue(["Mục tiêu doanh thu tháng", "Muc tieu doanh thu thang"]),
        ) || null,
      target_traffic_month:
        extractString(
          findValue(["Mục tiêu Traffic tháng", "Muc tieu Traffic thang"]),
        ) || null,
      kpi_progress_month: parseNum(
        findValue(["Tiến độ KPI tháng", "Tien do KPI thang"]),
      ),
      employee_status:
        extractString(findValue(["Tình trạng", "Tinh trang"])) || null,
      ...
      link_image:
        extractString(findValue(["Link ảnh", "Link anh", "flddOHyBPa"])) ||
        null,
    };
  }
  ...
  async getUserActivityReports(filters?: {
    date?: string;
    startDate?: string;
    endDate?: string;
    team?: string;
    ...
    if (filters?.requesterEmail) {
      const roleData = await this.cacheService.get(
        roleCacheKey,
        this.activityRoleCacheTtlMs,
        async () => {
          ...
          if (sysUser?.roles && sysUser.roles.length > 0) {
            ...
            else if (sysUser.roles.some((r) => r === ("LEADER" as any)))
              role = "leader";
          }
          ...
          if (!team) {
            ...
            const matched = recentKpis.find(
              (kpi: any) => this.extractEmailFromKpi(kpi) === requesterEmail,
            );
    ...
    const sharedData = await this.cacheService.get(
      sharedCacheKey,
      this.activitySharedCacheTtlMs,
      async () => {
        ...
        try {
          ...
          const teamFilterNormalized = filters?.team
          ...
          const isBitableKpiRowForSelection = (d: any) =>
            tsInRange(d, larkKpiStartOfDay, larkKpiEndOfDay);
          ...
          const isRangeFilter =
            filters?.team === "All Global" || filters?.team === "All VN";
          const dbTeamFilter =
            filters?.team && filters.team !== "All" && !isRangeFilter
              ? filters.team
              : null;
          ...
          const useDbTeamFilter = !!(
            dbTeamFilter && /^[\x00-\x7F]*$/.test(dbTeamFilter)
          );
          const normalizeTeamKey = (val: string | null | undefined) =>
            (val || "")
              ...
              .trim()
              .replace(/[\s-]+/g, "");
          const compactNameKey = (val: string | null | undefined) =>
            normName(val || "").replace(/\s+/g, "");
          const looseNameKey = (val: string | null | undefined) =>
            normName(val || "").replace(/[^a-z0-9]/g, "");
          const isDoDaTeamFilter =
            normalizeTeamKey(dbTeamFilter) === normalizeTeamKey("Đồ Da");
          ...
          for (const u of allUsersForTeam) {
            if (u.email && u.full_name) {
              ...
              if (ck)
                userEmailByNameCompact.set(ck, u.email.toLowerCase().trim());
            }
            if (u.image_url && u.email)
              userAvatarByEmail.set(u.email.toLowerCase().trim(), u.image_url);
          ...
          const leaderRoleByEmail = new Map<
            string,
            { role: string; team: string | null }
          >();
          const leaderRoleByName = new Map<
            string,
            { role: string; team: string | null }
          >();
          ...
          for (const emp of employeesMap.values()) {
            const eKey = String(emp.email || "")
              .toLowerCase()
              .trim();
            const nKey = normName(emp.full_name);
            const fromUsers =
              leaderRoleByEmail.get(eKey) || leaderRoleByName.get(nKey);
          ...
          employees.forEach((emp) => {
            if (emp.email)
              emailKeyMatchMap.set(emp.email.toLowerCase().trim(), emp);
            if (emp.full_name) {
          ...
          const teamFilterWhere =
            dbTeamFilter && useDbTeamFilter
              ? {
                  OR: [
                    {
                      team: {
                    ...
                    {
                      team: {
                        startsWith: `${dbTeamFilter},`,
                        mode: "insensitive" as any,
                      },
                    },
                    {
                      team: {
                        endsWith: `, ${dbTeamFilter}`,
                        mode: "insensitive" as any,
                      },
                    },
                    {
                      team: {
                        contains: `, ${dbTeamFilter},`,
                        mode: "insensitive" as any,
                      },
                    },
                  ],
          ...
          const [
              ...
              where: {
                OR: [
                  { month: { in: monthsInRange.flatMap((m) => m.formats) } },
                  {
                    ...
                    report_date: {
                      gte: monthsInRange[0]
                        ? getVietnamMonthBounds(
                            monthsInRange[0].year,
                            monthsInRange[0].monthNum,
                          ).start
                        : larkKpiStartOfDay,
                      lte:
                        monthsInRange.length > 0
                          ? getVietnamMonthBounds(
                              ...
                              monthsInRange[monthsInRange.length - 1].monthNum,
                            ).end
                          : larkKpiEndOfDay,
                    },
                  },
            ...
            (this.prisma as any).larkReportKPI.findMany({
              where: {
                report_date: {
                  gte: getVietnamMonthBounds(
                    monthsInRange[0].year,
                    monthsInRange[0].monthNum,
                  ).start,
                  lte: getVietnamMonthBounds(
                    monthsInRange[monthsInRange.length - 1].year,
                    monthsInRange[monthsInRange.length - 1].monthNum,
                  ).end,
                },
            ...
            this.prisma.larkTraffic.findMany({
              where: { date: { gte: memberReportStart, lte: memberReportEnd } },
            }),
          ...
          const dodaKpis = (dodaKpisRaw as any[])
            .map((r: any) => {
              ...
              return {
                ...
                kpi_day_percent:
                  r.kpi_day && r.kpi_day > 0
                    ? `${Math.round(((r.completed_day || 0) / r.kpi_day) * 100)}%`
                    : "0%",
                kpii_status:
                  (r.completed_day || 0) >= (r.kpi_day || 0)
                    ? "ĐẠT"
                    : "CHƯA ĐẠT",
          ...
          if (isDoDaTeamFilter) {
            ...
            for (const kpi of allKpiInDb as any[]) {
              const kpiEmail = (this.extractEmailFromKpi(kpi) || "")
                .toLowerCase()
                .trim();
              ...
              const matchedEmail =
                (kpiEmail ? kpiEmail : null) ||
                (kpiName ? userEmailByName.get(kpiName) : null) ||
                (kpiNameCompact
                  ? userEmailByNameCompact.get(kpiNameCompact)
                  : null) ||
                null;
              const avatar =
                (kpiEmail ? userAvatarByEmail.get(kpiEmail) : null) ||
                (kpiName ? userAvatarByName.get(kpiName) : null) ||
                (kpiNameCompact
                  ? userAvatarByNameCompact.get(kpiNameCompact)
                  : null) ||
          ...
          let kpiData = allKpiInDb.filter((k) => {
            ...
            if (!mStr) {
              ...
              return monthsInRange.some(
                (m) => vn.m === m.monthNum && vn.y === m.year,
              );
            }
            ...
            return monthsInRange.some((monthInfo) => {
              ...
              if (
                mDigits &&
                mDigits.some((d) => parseInt(d, 10) === monthInfo.monthNum)
              ) {
                // If multiple digits, assume one might be the year
                if (mDigits.length > 1) {
                  return mDigits.some(
                    (d) => parseInt(d, 10) === monthInfo.year,
                  );
                }
                return true;
              }
          ...
          const globalIndoOrDaiLoanKpis = kpiData.filter((k: any) => {
            ...
            return (
              t.includes("global - indo") ||
              t.includes("global-indo") ||
              t.includes("global indo") ||
              t.includes("global - đài") ||
              t.includes("global - dai") ||
              t.includes("global đài") ||
              t.includes("global dai")
            );
          });
          ...
          this.logger.debug(
            `[Optimization] Parallel fetch completed. Reports: ${reports.length}, KPIs: ${kpiData.length}`,
          );
          ...
          employees.forEach((emp: any) => {
            const st = (emp.employee_status || emp.status || "")
              .toLowerCase()
              .trim();
            const nKey = normName(emp.full_name);
            ...
            if (nKey && (nameCountsAll.get(nKey) || 0) === 1)
              fullStatusMap.set(nKey, st);
            if (eKey) fullStatusMap.set(eKey, st);
            if (emp.employee_id)
              fullStatusMap.set(String(emp.employee_id).trim(), st);
          });
          ...
          activeEmployees.forEach((emp: any) => {
            ...
            if (row.employee_id)
              employeeMap.set(String(row.employee_id).trim(), row);
          ...
          const matchesTeamFilter = (
            ...
            if (filter === "all global") {
              return teams.some(
                (t) => getRegionInternal(t.toLowerCase()) === "global",
              );
            }
            if (filter === "all vn") {
              return teams.some(
                (t) => getRegionInternal(t.toLowerCase()) === "vn",
              );
            }
          ...
          allChannelsInDb.forEach((h) => {
            const ownerName = h.owner?.full_name ?? null;
            if (ownerName) {
              ...
              const ownerEmail = h.owner?.email
                ? String(h.owner.email).toLowerCase().trim()
                : null;
              if (ownerEmail) {
                channelCountByEmail.set(
                  ownerEmail,
                  (channelCountByEmail.get(ownerEmail) || 0) + 1,
                );
              }
            }
            const teamNorm = (h.team?.name || "").toLowerCase().trim();
            let isMatch = false;
            if (!currentTeamFilter) isMatch = true;
              ...
              isMatch =
                normTeam === normCurrent ||
                normTeam.includes(normCurrent) ||
                normCurrent.includes(normTeam);
            }

            if (isMatch) {
              const region = getRegionInternal(h.team?.name || "");
          ...
          allTrafficInDb.forEach((t) => {
            ...
            const inferredEmail =
              (t.email ? String(t.email).toLowerCase().trim() : null) ||
              (perm?.email ? String(perm.email).toLowerCase().trim() : null);
            ...
            const mergeTraffic = (existing: any, current: any) => {
              ...
              res.total_traffic =
                (res.total_traffic || BigInt(0)) +
                (current.total_traffic || BigInt(0));
              platforms.forEach((p) => {
                const tk = `traffic_${p}`,
                  ck = `channel_${p}`,
                  ek = `evidence_${p}`;
                ...
                if (val > 0) {
                  let ev = [];
                  try {
                    if (current[ek]) ev = JSON.parse(current[ek]);
                  } catch (e) {}
                  res.details.push({
                    platform: p,
                    channel: current[ck] || "",
                    value: val,
                    evidences: (Array.isArray(ev) ? ev : []).filter((e) => e),
                  });
                }
              });
              return res;
            ...
            const merged = trafficMapByName.has(nameKey)
              ? mergeTraffic(trafficMapByName.get(nameKey), t)
              : mergeTraffic({ total_traffic: BigInt(0) }, t);
            trafficMapByName.set(nameKey, merged);

            if (inferredEmail) {
              const mergedMail = trafficMapByEmail.has(inferredEmail)
                ? mergeTraffic(trafficMapByEmail.get(inferredEmail), t)
                : mergeTraffic({ total_traffic: BigInt(0) }, t);
          ...
          const isReportKpiOnAuxDay = (d: any) =>
            tsInRange(d, memberReportStart, memberReportEnd);
          ...
          dailyReportKpis.forEach((rk) => {
            ...
            const mergeUpdate = (existing: any, current: any) => {
              ...
              const existingIsTarget = isReportKpiOnAuxDay(
              ...
              if (currentIsTarget) {
                ...
                res.completed_day = Math.max(
                  Number(res.completed_day) || 0,
                  Number(current.completed_day) || 0,
                );
                if (
                  !existing.report_date ||
                  (current.report_date &&
                    new Date(current.report_date) >
                      new Date(existing.report_date))
                ) {
                  res.report_date = current.report_date;
                }
              }

              res.kpi_day = Math.max(
                Number(res.kpi_day) || 0,
                Number(current.kpi_day) || 0,
              );
              res.task_auto =
                (Number(res.task_auto) || 0) + (Number(current.task_auto) || 0);
              res.task_new =
                (Number(res.task_new) || 0) + (Number(current.task_new) || 0);
            ...
            if (emailKey) {
              ...
              reportKpiMapByEmail.set(
                key,
                reportKpiMapByEmail.has(key)
                  ? mergeUpdate(reportKpiMapByEmail.get(key), rk)
                  : { ...rk },
              );
            }
            if (nameKey) {
              const key = `${nameKey}_${timeKey}`;
              reportKpiMapByName.set(
                key,
                reportKpiMapByName.has(key)
                  ? mergeUpdate(reportKpiMapByName.get(key), rk)
                  : { ...rk },
              );
            }
          });
          ...
          monthlyReportKpis.forEach((rk) => {
            ...
            if (emailKey) {
              ...
              if (
                !existing ||
                new Date(rk.report_date) > new Date(existing.report_date)
              ) {
          ...
          const getAggKey = (
            pKey: string,
            teamNorm: string,
            mInfo: { monthNum: number; year: number },
          ) =>
            selectedSingleDay
              ? `${pKey}_T${mInfo.monthNum}_${mInfo.year}`
              : `${pKey}_${teamNorm}_T${mInfo.monthNum}_${mInfo.year}`;
          ...
          if (selectedSingleDay) {
            ...
            activeEmployees.forEach((emp) => {
              ...
              if (!matchesTeamFilter(checklistTeams, teamFilterNormalized))
                return;
              ...
              const perfTeam =
                ...
                (empNameKey
                  ? personPerformanceTeamMap.get(empNameKey)
                  : null) ||
                "";
              const pmk = getAggKey(
                pKey,
                normalizeTeamKey(checklistTeams[0] || "khac"),
                monthInfo,
              );
            ...
            for (const kpi of targetDayKpis) {
              ...
              if (
                ...
                !isActiveEmployeeStatus(
                  authUser.employee_status || authUser.status,
                )
              ) {
                continue;
              }
              ...
              if (!kpisForAggregation.has(pmk)) {
                ...
                existing.kpi_day =
                  Number(kpi.kpi_day) > 0
                    ? Number(kpi.kpi_day)
                    : Number(existing.kpi_day || 0);
                existing.completed_day =
                  Number(kpi.completed_day) ||
                  Number(existing.completed_day || 0);
          ...
          const resolvePersonKey = (opts: {
            ...
            const emailKey = String(opts.email || "")
              .toLowerCase()
              .trim();
            if (emailKey) {
              const byEmail = emailKeyMatchMap.get(emailKey);
              if (byEmail?.email)
                return String(byEmail.email).toLowerCase().trim();
              return emailKey;
            }
            const empId = String(opts.employeeId || "").trim();
            if (empId) {
              const byId =
                larkUserIdMatchMap.get(empId) || employeeMap.get(empId);
            ...
            if (nameKey) {
              ...
              if (byName?.email)
                return String(byName.email).toLowerCase().trim();
              return nameKey;
            }
          ...
          const applyTargetDayKpiToAggregation = () => {
            ...
            for (const kpi of targetDayKpis) {
              ...
              if (
                !prev ||
                new Date(kpi.report_date || 0).getTime() >=
                  new Date(prev.report_date || 0).getTime()
              ) {
          ...
          kpiData.forEach((kpi) => {
            ...
            const kpiEmpId = kpi.employee_id
              ? String(kpi.employee_id).trim()
              : null;
            const authUser =
              (emailKey ? emailKeyMatchMap.get(emailKey) : null) ||
            ...
            const matchedMonth =
              monthsInRange.find((mInfo) => {
                if (mInfo.formats.includes(mStr)) return true;
                const mDigits = mStr.match(/\d+/g);
                return (
                  mDigits &&
                  mDigits.some((d) => parseInt(d, 10) === mInfo.monthNum)
                );
              }) || monthsInRange[0];
            ...
            const existing = kpisForAggregation.get(pmk);
            const currentIsTarget = isBitableKpiRowForSelection(
              kpi.report_date,
            );
            const existingIsTarget = isBitableKpiRowForSelection(
              existing.report_date,
            );
            ...
            if (currentIsTarget) {
              ...
              existing.completed_day = Math.max(
                Number(existing.completed_day) || 0,
                Number(kpi.completed_day) || 0,
              );
              existing.kpi_day = Math.max(
                Number(existing.kpi_day) || 0,
                Number(kpi.kpi_day) || 0,
              );
            }

            existing.kpi_month = Math.max(
              Number(existing.kpi_month) || 0,
              Number(kpi.kpi_month) || 0,
            );
            existing.completed_month = Math.max(
              Number(existing.completed_month) || 0,
              Number(kpi.completed_month) || 0,
            );
          ...
          reports.forEach((r) => {
            ...
            if (
              !authoritativeUser &&
              (nameKey === "chung do" || emailKey === "dochung2741@gmail.com")
            ) {
              authoritativeUser =
                nameKeyMatchMap.get("do dang chung") ||
                emailKeyMatchMap.get("dochung2741@gmail.com");
            }

            if (
              authoritativeUser &&
              !isActiveEmployeeStatus(
                authoritativeUser.employee_status || authoritativeUser.status,
              )
            ) {
              return;
            }
            ...
            const pKey = authoritativeUser
              ? authoritativeUser.email?.toLowerCase().trim() ||
                normName(authoritativeUser.full_name)
            ...
            teams.forEach((team) => {
              ...
              const personMonthKey = getAggKey(pKey, teamNorm, {
                monthNum: vn.m,
                year: vn.y,
              });
              ...
              if (!kpisForAggregation.has(personMonthKey)) {
                kpisForAggregation.set(personMonthKey, {
                  ...
                  team:
                    (emailKey
                      ? personPerformanceTeamMap.get(emailKey)
                      : null) ||
                ...
                if (authoritativeUser?.full_name)
                  existing.name = authoritativeUser.full_name;
                if (authoritativeUser?.role)
                  existing.role = authoritativeUser.role;
                if (authoritativeUser?.email)
                  existing.email = authoritativeUser.email;
          ...
          reports.forEach((r) => {
            ...
            let authUser =
              (rEmail ? emailKeyMatchMap.get(rEmail) : null) ||
              (rName ? nameKeyMatchMap.get(rName) : null);

            if (
              !authUser &&
              (rName === "chung do" || rEmail === "dochung2741@gmail.com")
            ) {
              authUser =
                nameKeyMatchMap.get("do dang chung") ||
                emailKeyMatchMap.get("dochung2741@gmail.com");
            ...
            rTeams.forEach((t) => {
              ...
              if (authUser?.email)
                reportsByTeamMap.set(
                  `${authUser.email.toLowerCase().trim()}_${rTeamNorm}`,
                  r,
                );
              if (authUser?.full_name)
                reportsByTeamMap.set(
                  `${normName(authUser.full_name)}_${rTeamNorm}`,
                  r,
                );
            });
          });

          const allResults = Array.from(kpisForAggregation.values()).map(
            (kpi) => {
              const nameKey =
                kpi.name
                  ?.toLowerCase()
              ...
              const personEmp =
                ...
                (emailKey ? employeeMap.get(emailKey) : null);
              const permEmailKey = personEmp?.email
                ? personEmp.email.toLowerCase().trim()
                : null;
              ...
              const preliminaryChecklistTeamStr =
                ...
                (nameKey ? userTeamByName.get(nameKey) : null) ||
                "";
              const preliminaryChecklistTeams = splitTeamList(
                preliminaryChecklistTeamStr,
              );
              const performanceTeam =
                String(kpi.team || "").trim() ||
                (emailLookupKey
                  ? personPerformanceTeamMap.get(emailLookupKey)
                  : null) ||
                (nameKey ? personPerformanceTeamMap.get(nameKey) : null) ||
                "";
              ...
              const reportTeamCandidates = preliminaryChecklistTeams.length
                ...
                : splitTeamList(
                    preliminaryChecklistTeamStr || performanceTeam || "Khác",
                  );
              for (const ct of reportTeamCandidates) {
                const ctNorm = normalizeTeamKey(ct);
                report =
                  (emailLookupKey
                    ? reportsByTeamMap.get(`${emailLookupKey}_${ctNorm}`)
                    : null) ||
                  (nameKey
                    ? reportsByTeamMap.get(`${nameKey}_${ctNorm}`)
                    : null);
                if (report) break;
              }
              ...
              const reportEmailLookupKey = report?.email
                ? String(report.email).toLowerCase().trim()
                : null;
              const trustedEmailKey = emailLookupKey || reportEmailLookupKey;

              const empByEmpId = trimmedEmpId
                ? employeeMap.get(trimmedEmpId)
                : null;
              const empIdNameKey = empByEmpId
                ? normName(empByEmpId.name || "")
                : null;
              const employee =
              ...
              const resolvedByEmail = !!(
                trustedEmailKey && employeeMap.get(trustedEmailKey)
              );
              const resolvedByName =
                !resolvedByEmail && !!employeeMap.get(nameKey);
              const resolvedByEmpId =
                !resolvedByEmail &&
                !resolvedByName &&
                !!(empByEmpId && empIdNameKey === nameKey);
              ...
              // RESIGNED FILTERING
              const employeeStatusDirect = String(employee?.status || "")
                .toLowerCase()
                .trim();
              const currentStatus =
                employeeStatusDirect ||
                fullStatusMap.get(emailLookupKey || "") ||
                (!employee ? fullStatusMap.get(trimmedEmpId || "") : "") ||
                fullStatusMap.get(nameKey) ||
                "";
              const kpiEmpStatus = (kpi.employee_status || "")
                .toLowerCase()
                .trim();
              const kpiState = (kpi.state || "").toLowerCase().trim();

              const isResigned =
                employeeStatusDirect.includes("nghỉ") ||
              ...
              const effectiveTeam =
                performanceTeam || checklistTeams[0] || "Khác";
              const checklistSourceTeam =
                checklistTeamStr || checklistTeams.join(", ") || effectiveTeam;
              ...
              if (!teamFilterNormalized) {
                ...
                isMatchForRanking = perfPool.some(
                  (t) => getRegionInternal(t.toLowerCase()) === "global",
                );
                ...
                isMatchForRanking = matchesTeamFilter(
                  perfPool,
              ...
              const isChecklistTeamMatch = matchesTeamFilter(
                checklistTeams.length
                  ? checklistTeams
                  : splitTeamList(checklistSourceTeam),
                teamFilterNormalized,
              );

              const personEmailForSelf =
                report?.email || personEmp?.email || kpi.email;
              const isSelf =
                filters?.requesterEmail &&
                personEmailForSelf &&
                personEmailForSelf.toLowerCase().trim() ===
                  filters.requesterEmail.toLowerCase().trim();
              ...
              const isAuthorized = selectedSingleDay
                ...
                : hasExplicitTeamFilter
                  ? isChecklistTeamMatch
                  : isChecklistTeamMatch || isSelf;
              ...
              if (answersData && typeof answersData === "object") {
                checklist.fb =
                  answersData["Bạn đã đăng video lên FB chưa?"] === true ||
                  answersData[
                    "Báo cáo Lark - Bạn đã đăng video lên FB chưa?"
                  ] === true ||
                  false;
                checklist.ig =
                  answersData["Bạn đã đăng video lên IG chưa?"] === true ||
                  answersData[
                    "Báo cáo Lark - Bạn đã đăng video lên IG chưa?"
                  ] === true ||
                  false;
                checklist.tiktok =
                  answersData["Bạn đã đăng video lên Tiktok chưa?"] === true ||
                  answersData[
                    "Báo cáo Lark - Bạn đã đăng video lên Tiktok chưa?"
                  ] === true ||
                  false;
                checklist.youtube =
                  answersData["Bạn đã đăng video lên Youtube chưa?"] === true ||
                  answersData[
                    "Báo cáo Lark - Bạn đã đăng video lên Youtube chưa?"
                  ] === true ||
                  false;
                checklist.lark =
                  answersData[
                  ...
                  answersData[
                    "Báo cáo Lark - Bạn đã báo cáo đầy đủ thông tin công việc trên lark chưa?"
                  ] === true ||
                  false;
                checklist.caption =
                  answersData[
                  ...
                  answersData[
                    "Báo cáo Lark - Bạn đã check lại caption và hagtag video chưa?"
                  ] === true;
              }
              ...
              const matchedMonthFinal =
                monthsInRange.find((m) => m.formats.includes(mStrFinal)) ||
                monthsInRange[0];
              ...
              const rKpi =
                (emailKeyFinal
                  ? reportKpiMapByEmail.get(`${emailKeyFinal}_${timeKey}`)
                  : null) ||
                (nkFinal
                  ? reportKpiMapByName.get(`${nkFinal}_${timeKey}`)
                  : null);
              ...
              const rKpiEmailKey = report?.email
                ? `${report.email.toLowerCase().trim()}_${timeKey}`
                : null;
              const rKpiNameKey = `${nameKey}_${timeKey}`;

              const reportKpi =
                (rKpiEmailKey ? reportKpiMapByEmail.get(rKpiEmailKey) : null) ||
                reportKpiMapByName.get(rKpiNameKey);

              // --- FIX: Lookup monthly stable KPI for Summary Cards ---
              const monthlyReportKpi =
                (rKpiEmailKey
                  ? monthlyKpiMapByEmail.get(rKpiEmailKey)
                  : null) || monthlyKpiMapByName.get(rKpiNameKey);

              const nowVn = getVietnamParts();
              const isCurrentMonth =
                matchedMonth.monthNum === nowVn.m &&
                matchedMonth.year === nowVn.y;
              const incrementalTraffic =
                isCurrentMonth && answersData
                  ? Number(
                      answersData[
                        "Bạn đã đạt bao nhiêu traffic cho video mới?"
                      ],
                    ) || 0
                  : 0;
              const incrementalRevenue =
                isCurrentMonth && answersData
                  ? Number(
                      answersData["Bạn đã đạt doanh thu của bao nhiêu video?"],
                    ) || 0
                  : 0;
              const isRange =
                filters?.timeType &&
                !["today", "yesterday"].includes(filters.timeType);
              ...
              const isViewingToday =
                !filters?.timeType || filters.timeType === "today";

              const personEmail =
                report?.email || reportKpi?.email || personEmp?.email;
              const normalizedEmail = personEmail?.toLowerCase().trim();
              const channelsOwnedCount = Math.max(
                channelCountByNormOwner.get(nameKey) || 0,
                normalizedEmail
                  ? channelCountByEmail.get(normalizedEmail) || 0
                  : 0,
              );
              const needsTraffic = channelsOwnedCount > 0;
              const trafficObj =
                (normalizedEmail
                  ? trafficMapByEmail.get(normalizedEmail)
                  : null) || trafficMapByName.get(nameKey);
              ...
              if (baseReported) {
                if (needsTraffic) {
                  effectiveStatus = hasTraffic
                    ? "ĐÃ BÁO CÁO ĐỦ"
                    : "CHƯA BÁO CÁO TRAFFIC";
                } else {
              ...
              let effectiveDate =
                report?.created_at ||
                report?.date ||
                reportKpi?.created_at ||
                reportKpi?.report_date ||
                null;
              if (hasTraffic && trafficObj) {
                effectiveDate =
                  trafficObj.created_at || trafficObj.date || effectiveDate;
              }
              ...
              const resolvedAvatar =
                ...
                this.convertDriveUrl(
                  (emailLookupKey
                    ? userAvatarByEmail.get(emailLookupKey)
                    : null) ||
                    (nameKey ? userAvatarByName.get(nameKey) : null) ||
                    (compactNameKey(kpi.name || "")
                      ? userAvatarByNameCompact.get(
                          compactNameKey(kpi.name || ""),
                        )
                      : null) ||
                    (looseNameKey(kpi.name || "")
                      ? userAvatarByNameLoose.get(looseNameKey(kpi.name || ""))
                      : null),
                ) ||
              ...
              return {
                ...
                position:
                  employee?.position ||
                  (employee?.role === "leader" ? "Leader" : "Member"),
                // Role: ưu tiên lấy từ chính employee (bảng users)
                // Role: ưu tiên lấy từ chính employee (bảng users)
                role:
                  employee?.role || personEmp?.role || kpi._empRole || "member",
                ...
                checklist,
                answers: answersData,
                videoCount: answersData
                  ? Number(
                      answersData[
                        Object.keys(answersData).find((k) =>
                ...
                completed_month: monthlyReportKpi
                  ? Number(monthlyReportKpi.completed_month)
                  : kpi.completed_month || 0,
                // Stable monthly traffic/revenue for the Summary Cards:
                traffic_range:
                  (monthlyReportKpi
                    ? Number(monthlyReportKpi.traffic_month || 0)
                    : Number(kpi.traffic_month || 0)) + incrementalTraffic,
                revenue_range:
                  (monthlyReportKpi
                    ? Number(monthlyReportKpi.revenue_month || 0)
                    : Number(kpi.revenue_month || 0)) + incrementalRevenue,
                revenueToday:
                  (reportKpi ? Number(reportKpi.revenue_day || 0) : 0) +
                  incrementalRevenue,
                task_progress: reportKpi
                  ...
                  : {
                      ...
                      kpi_status: isViewingToday
                        ? "N/A"
                        : kpi.kpii_status || "N/A",
                    },
                traffic_month: Math.max(
                  Number(monthlyReportKpi?.traffic_month || 0),
                  Number(kpi.traffic_month || 0),
                ),
                revenue_month: Math.max(
                  Number(monthlyReportKpi?.revenue_month || 0),
                  Number(kpi.revenue_month || 0),
                ),
                trafficTarget: parseInt(kpi.target_traffic_month || "0") || 0,
                revenueTarget: parseInt(kpi.target_revenue_month || "0") || 0,
                monthlyProgress:
                  kpi.kpi_progress_month !== null
                    ? Math.round(Number(kpi.kpi_progress_month) * 100)
                    : (kpi.kpi_month || 0) > 0
                      ? Math.round(
                          ((kpi.completed_month || 0) / kpi.kpi_month) * 100,
                        )
                      : 0,
          ...
          reports.forEach((report) => {
            const rEmailKey = report.email
              ? report.email.toLowerCase().trim()
              : "";
            const rNameKey = report.name
              ? report.name
                  .toLowerCase()
                  .normalize("NFD")
                  .replace(/[\u0300-\u036f]/g, "")
            ...
            const isAlreadyIncluded = allResults.some((r) => {
              ...
              const n = r.name
                ...
                : "";
              return (
                (rEmailKey && e === rEmailKey) || (rNameKey && n === rNameKey)
              );
            });
            ...
            if (!isAlreadyIncluded) {
              ...
              const performanceTeam =
                ...
                "";
              const userTeamsRep = checklistTeams.length
                ? checklistTeams
                : reportOwnTeam
                  ? [reportOwnTeam]
                  : [];
              ...
              const fallbackAvatar =
                ...
                this.convertDriveUrl(
                  (rEmailKey ? userAvatarByEmail.get(rEmailKey) : null) ||
                    ...
                    (compactNameKey(report.name || "")
                      ? userAvatarByNameCompact.get(
                          compactNameKey(report.name || ""),
                        )
                      : null) ||
                    (looseNameKey(report.name || "")
                      ? userAvatarByNameLoose.get(
                          looseNameKey(report.name || ""),
                        )
                      : null),
                ) ||
              ...
              allResults.push({
                ...
                personKey:
                  rEmailKey ||
                  `${rNameKey || "unknown"}|${displayTeamRep.toLowerCase().trim()}` ||
                  report.id,
                name: report.name,
          ...
          // #endregion
          const preGroupDupStats = {
            duplicateKeys: 0,
            duplicateRows: 0,
            duplicateRowsWithNonZeroDaily: 0,
          };
            ...
            .forEach((r: any) => {
              const baseKey =
                ...
                r.name
                  ...
                  .replace(/\s+/g, " ") ||
                r.employee_id;
              const rowTeamNorm = normalizeTeamKey(
                String(r.team || "khac").toLowerCase(),
              );
          ...
          preGroupByKey.forEach((rows) => {
            ...
            if (
              rows.some(
                (x: any) =>
                  Number(x?.kpi_day || 0) > 0 ||
                  Number(x?.completed_day || 0) > 0,
              )
            ) {
          ...
          allResults
            ...
            .forEach((r) => {
              ...
              const statusStr = String(r.employee_status || "")
                .toLowerCase()
                .trim();
              if (
                statusStr.includes("nghỉ") ||
                statusStr.includes("off") ||
                statusStr.includes("khóa")
              ) {
                return; // skip this off/inactive member
              }
              if (
                r.name &&
                (r.name.toLowerCase().includes("nghỉ") ||
              ...
              const authoritativeUser =
                (emailKeyRep ? employeeMap.get(emailKeyRep) : null) ||
                (nameKeyRep ? employeeMap.get(nameKeyRep) : null);
              ...
              if (!groupedResults.has(key)) {
                ...
                if (authoritativeUser) {
                  ...
                  newObj.checklist_source_team =
                    authoritativeUser.team ||
                    r.checklist_source_team ||
                    newObj.checklist_source_team;
                  ...
                  newObj.position =
                    authoritativeUser.position ||
                    (authoritativeUser.role === "leader" ? "Leader" : "Member");
                }
                groupedResults.set(key, newObj);
              } else {
                const existing = groupedResults.get(key);
                if (
                  (r as any).hasExactDayKpi &&
                  !(existing as any).hasExactDayKpi
                ) {
                ...
                existing.revenueToday =
                  (existing.revenueToday || 0) + (r.revenueToday || 0);
                ...
                if ((r as any).hasExactDayKpi) {
                  ...
                  existing.dailyGoal =
                    r.dailyGoal > 0 ? r.dailyGoal : existing.dailyGoal;
                }

                existing.kpi_month = Math.max(existing.kpi_month, r.kpi_month);
                existing.trafficTarget = Math.max(
                  existing.trafficTarget,
                  r.trafficTarget,
                );
                existing.revenueTarget = Math.max(
                  existing.revenueTarget,
                  r.revenueTarget,
                );

                existing.channelCount = Math.max(
                ...
                if (
                  r.date &&
                  (!existing.date || new Date(r.date) > new Date(existing.date))
                ) {
          ...
          if (isDoDaTeamFilter) {
            this.logger.log(
              `[DoDa-Avatar] lastmile: allValidResults=${allValidResults.length}, allUsersForTeam=${allUsersForTeam.length}`,
            );
          ...
          const preGroupNonZeroDaily = allResults.filter(
            (r: any) =>
              r !== null &&
              (Number(r?.kpi_day || 0) > 0 ||
                Number(r?.completed_day || 0) > 0),
          ).length;
          const postGroupNonZeroDaily = allValidResults.filter(
            (r: any) =>
              Number(r?.kpi_day || 0) > 0 || Number(r?.completed_day || 0) > 0,
          ).length;
          ...
          const zeroDailyCount = allValidResults.filter(
            (r: any) =>
              Number(r?.kpi_day || 0) === 0 &&
              Number(r?.completed_day || 0) === 0,
          ).length;
          ...
          const multiTeamResultDebug = allValidResults
            ...
            .map((r: any) => ({
              ...
              team: r.team,
              checklistMarked: !!(
                r.checklist && Object.values(r.checklist).some(Boolean)
              ),
              status: r.status,
            }));

          // Phân tách dữ liệu Báo cáo và BXH
          const combinedResults = allValidResults.filter(
            (r) => r.isAuthorizedForReport,
          );
          const rankingList = allValidResults.filter(
            (r) => r.isMatchForRanking,
          );
          ...
          rankingList.forEach((r) => {
            const employee =
              employeeMap.get(
                r.name?.toLowerCase().trim().replace(/\s+/g, " ") || "",
              ) ||
              (r.employee_id ? employeeMap.get(r.employee_id.trim()) : null);
            const empStatus = (
              employee?.status ||
              ...
              .trim();
            const isResigned =
              empStatus.includes("nghỉ") ||
              empStatus.includes("off") ||
              empStatus.includes("khóa");
          ...
          const trafficRanking = rankingList
            .sort(
              (a, b) =>
                Number(b.traffic_range || 0) - Number(a.traffic_range || 0),
            )
            .slice(0, 10)
            .map((kpi, index) => {
              const nameKey =
              ...
              return {
                ...
                avatar:
                  ...
                  this.convertDriveUrl(
                    (kpi.email
                      ? userAvatarByEmail.get(
                          String(kpi.email).toLowerCase().trim(),
                        )
                      : null) ||
                      (nameKey ? userAvatarByName.get(nameKey) : null),
                  ) ||
          ...
          const revenueRanking = rankingList
            .sort(
              (a, b) =>
                Number(b.revenue_range || 0) - Number(a.revenue_range || 0),
            )
            .slice(0, 10)
            .map((kpi, index) => {
              const nameKey =
          ...
          allKpiForMonth.forEach((k) => {
            ...
            if (
              !globalKpis.has(key) ||
              (k.completed_month || 0) >
                (globalKpis.get(key).completed_month || 0)
            ) {
          ...
          const teamContributions = Object.entries(teamBreakdown)
            .map(([team, stats]: [string, any]) => ({
              team,
              videoPct: globalTotals.videos
                ? Math.round((stats.videos / globalTotals.videos) * 100)
                : 0,
              trafficPct: globalTotals.traffic
                ? Math.round((stats.traffic / globalTotals.traffic) * 100)
                : 0,
              revenuePct: globalTotals.revenue
                ? Math.round((stats.revenue / globalTotals.revenue) * 100)
                : 0,
              channels: stats.channels || 0,
              channelPct: globalTotals.channels
                ? Math.round(
                    ((stats.channels || 0) / globalTotals.channels) * 100,
                  )
                : 0,
            }))
          ...
          const groupTotals = {
            global: {
              ...
              traffic: 0,
              revenue: 0,
              channels: regionalChannelCounts.global,
            },
            vn: {
              videos: 0,
              traffic: 0,
              revenue: 0,
              channels: regionalChannelCounts.vn,
            },
          };
          ...
          globalTotals.channels =
            groupTotals.global.channels + groupTotals.vn.channels;
          ...
          const groupContributions = {
            global: {
              ...
              videoPct: globalTotals.videos
                ? Math.round(
                    (groupTotals.global.videos / globalTotals.videos) * 100,
                  )
                : 0,
              trafficPct: globalTotals.traffic
                ? Math.round(
                    (groupTotals.global.traffic / globalTotals.traffic) * 100,
                  )
                : 0,
              revenuePct: globalTotals.revenue
                ? Math.round(
                    (groupTotals.global.revenue / globalTotals.revenue) * 100,
                  )
                : 0,
              channelPct: globalTotals.channels
                ? Math.round(
                    (groupTotals.global.channels / globalTotals.channels) * 100,
                  )
                : 0,
            },
            vn: {
              ...
              videoPct: globalTotals.videos
                ? Math.round(
                    (groupTotals.vn.videos / globalTotals.videos) * 100,
                  )
                : 0,
              trafficPct: globalTotals.traffic
                ? Math.round(
                    (groupTotals.vn.traffic / globalTotals.traffic) * 100,
                  )
                : 0,
              revenuePct: globalTotals.revenue
                ? Math.round(
                    (groupTotals.vn.revenue / globalTotals.revenue) * 100,
                  )
                : 0,
              channelPct: globalTotals.channels
                ? Math.round(
                    (groupTotals.vn.channels / globalTotals.channels) * 100,
                  )
                : 0,
            },
          };
  ...
  async getUserReportDetails(email: string, dateStr: string) {
    ...
    const report =
      reportCandidates.find((r: any) => isMatchedPerson(r.email, r.name)) ||
      null;
    const trafficRecords = trafficCandidates.filter((t: any) =>
      isMatchedPerson(t.email, t.name),
    );
      ...
      const buildDetails = (rec: any) => {
        platforms.forEach((p) => {
          ...
          if (val > 0) {
            let ev = [];
            try {
              if (rec[ek]) ev = JSON.parse(rec[ek]);
            } catch (e) {}
            details.push({
              platform: p,
              channel: rec[ck] || "",
      ...
      if (trafficRecords.length > 1) {
        for (let i = 1; i < trafficRecords.length; i++) {
            ...
            if (rec[ek]) {
              try {
                ...
                if (Array.isArray(newE))
                  traffic[ek] = JSON.stringify([...curr, ...newE]);
              } catch (e) {}
            }
          });
          ...
          if (rec.evidence_files) {
            try {
              const currF = traffic.evidence_files
                ? JSON.parse(traffic.evidence_files)
                : [];
              const newF = JSON.parse(rec.evidence_files);
              if (Array.isArray(newF))
                traffic.evidence_files = JSON.stringify([...currF, ...newF]);
            } catch (e) {}
          }
          buildDetails(rec);
        }
    ...
    const reportedTeams = Array.from(
      new Set(trafficRecords.map((t) => t.team).filter(Boolean)),
    );
  ...
  convertDriveUrl(url: string | null | undefined): string | null {
    ...
    if (trimmed.includes("drive.google.com")) {
      const match =
        trimmed.match(/\/d\/([^/]+)/) || trimmed.match(/id=([^&]+)/);
      if (match && match[1]) {
  ...
  async getMedia(
    ...
    const candidates = this.buildMediaDownloadCandidates(
      normalized.mediaId,
      normalized.extra,
    );
    ...
    for (const candidate of candidates) {
      const url = this.buildLarkMediaDownloadUrl(
        candidate.mediaId,
        candidate.extra,
      );
      try {
  ...
  private buildMediaDownloadCandidates(
    mediaId: string,
    extra?: string,
  ): Array<{ mediaId: string; extra?: string }> {
    ...
    const addCandidate = (candidateExtra?: string) => {
      ...
      if (!exists) {
        if (normalizedExtra)
          candidates.push({ mediaId, extra: normalizedExtra });
    ...
    if (extra) {
      ...
      if (
        ...
        typeof parsedExtra.bitablePerm === "object" &&
        parsedExtra.bitablePerm.rev != null
      ) {
        const withoutRev = {
          ...parsedExtra,
          bitablePerm: {
  ...
  private parseLarkMediaRef(
    rawUrl: string,
  ): { mediaId: string; extra?: string } | null {
    if (!rawUrl) return null;
    const trimmed = rawUrl.trim();

    // High confidence: It's a direct Lark token (file_token_..., obj_..., tmp_...)
    if (
      trimmed.startsWith("file_token_") ||
      trimmed.startsWith("obj_") ||
      trimmed.startsWith("tmp_")
    ) {
  ...
  private normalizeMediaRequest(
    mediaIdRaw: string,
    extraRaw?: string,
  ): { mediaId: string; extra?: string } {
  ...
  async syncPermissionData() {
    if (!this.PERMISSION_TABLE_ID) {
      this.logger.warn(
        "LARK_PERMISSION_TABLE_ID not configured, skipping sync.",
      );
      return;
    }
    ...
    try {
      const records = await this.fetchAllRecords(
        this.REPORT_BASE_ID,
        this.PERMISSION_TABLE_ID,
      );
      this.logger.log(
        `Fetched ${records.length} records from Permission Table. Syncing to database...`,
      );
      ...
      if (records.length > 0) {
        this.logger.log(
          `[DEBUG PERMISSION MAPPING] Keys: ${Object.keys(records[0].fields).join(", ")}`,
        );
        this.logger.log(
          `[DEBUG PERMISSION MAPPING] Values: ${JSON.stringify(records[0].fields)}`,
        );
      }
      ...
      for (const record of records) {
        ...
        const email = fields["Email"] || null;
        const name =
          fields["HoTen"] || fields["Họ Tên"] || fields["Name"] || null;
        const maPin =
          fields["MaPin"] || fields["Mã Pin"] || fields["Mã pin"] || null;
        ...
        if (
          lowStatus.includes("nghỉ") ||
          lowStatus.includes("off") ||
          lowStatus.includes("khóa")
        ) {
          employee_status = "off";
        }
        ...
        if (
          employeeRaw &&
          Array.isArray(employeeRaw) &&
          employeeRaw.length > 0
        ) {
          avatarUrl = employeeRaw[0].avatar_url || null;
          empId = employeeRaw[0].id || null;
        }
        const imageField =
          fields["Anh"] || fields["Ảnh"] || fields["Avatar"] || fields["Image"];
        if (
          typeof imageField === "string" &&
          imageField.trim().startsWith("http")
        ) {
        ...
        const fullName =
          (typeof name === "string" && name.trim().length > 0
            ? name.trim()
            : null) || (email ? email.split("@")[0] : "Unknown");

        const finalEmail = email
          ? email.trim().toLowerCase()
          : `${fullName.toLowerCase().replace(/[^a-z0-9]/g, "")}_${Date.now()}@noemail.com`;
        ...
        try {
          ...
          if (!existingUser && name) {
            const byName = await this.prisma.user.findFirst({
              where: {
                full_name: { equals: name.trim(), mode: "insensitive" },
              },
              select: {
                id: true,
                roles: true,
            ...
            if (byName) {
              const incomingEmailNorm = email
                ? email.trim().toLowerCase()
                : null;
              const foundEmailNorm = byName.email.toLowerCase();
              const emailsDiffer =
                incomingEmailNorm && incomingEmailNorm !== foundEmailNorm;
          ...
          if (existingUser) {
            ...
            const isRoleDowngrade =
              !!currentHighRole && userRoles[0] === "MEMBER";
            if (isRoleDowngrade) {
              this.logger.warn(
          ...
          this.logger.warn(
            `[PermSync] Could not upsert user for email/name ${email || name}: ${userErr.message}`,
          );
        }
      }
      ...
      if (syncedUserIds.length > 0) {
        ...
        this.logger.log(
          `Marked ${offUpdateResult.count} unsynced users as 'OFF'.`,
        );
      }

      this.logger.log(
        `Lark Permission sync completed. records: ${records.length}. users upserted: ${syncedUsers}, skipped (no email or name): ${skippedNoEmail}.`,
      );
    } catch (error) {
  ...
  async getPersonalHistory(requesterEmail: string, targetName?: string) {
    ...
    return this.cacheService.get(cacheKey, 5 * 60 * 1000, async () => {
      try {
        ...
        if (
          !userTeam &&
          (requesterRole === "leader" || requesterRole === "member")
        ) {
          ...
          if (lastReport?.team) {
            ...
            this.logger.debug(
              `Resolved team for ${userName} (${requesterRole}) from Report: ${userTeam}`,
            );
          }
        }
        ...
        if (targetName && targetName.trim()) {
          ...
          const targetUser = await this.prisma.user.findFirst({
            where: {
              full_name: { contains: targetName.trim(), mode: "insensitive" },
            },
          });

          if (targetUser) {
            const empStatus = (targetUser.employee_status || "")
              .toLowerCase()
              .trim();
            if (
              empStatus === "đã nghỉ" ||
            ...
            if (isAdmin || isManager || (isLeader && isSameTeam)) {
              ...
              this.logger.warn(
                `Unauthorized: ${requesterEmail} (${requesterRole}) tried to access ${targetName}`,
              );
        ...
        kpis.forEach((kpi) => {
          const monthStr =
            kpi.month || kpi.created_at.toISOString().substring(0, 7);
        ...
        const history = Array.from(monthlyData.values()).sort(
        ...
        if (allTeamKpis.length === 0) {
          ...
          if (latestKpi && latestKpi.month) {
            ...
            if (mDigits) {
              ...
              this.logger.log(
                `No data for T${new Date().getMonth() + 1}, falling back to month ${targetMonthNum}`,
              );
            }
          }
        }
        ...
        const currentMonthKpi =
          allTeamKpis
            .filter(
              (k) =>
                k.name?.toLowerCase().trim().replace(/\s+/g, " ") ===
                userName.toLowerCase().trim().replace(/\s+/g, " "),
            )
            .sort(
              (a, b) => (b.completed_month || 0) - (a.completed_month || 0),
            )[0] || null;
        ...
        try {
          ...
          allTeamKpis.forEach((k) => {
            ...
            if (
              !companyLatestMap.has(key) ||
              (k.completed_month || 0) >
                (companyLatestMap.get(key).completed_month || 0)
            ) {
          ...
          const companyChannelsCount = await this.prisma.channel
            .count()
            .catch(() => 0);
          ...
          this.logger.log(
            `Calculated Company Stats: ${JSON.stringify(companyStats)} from ${companyLatestMap.size} unique records`,
          );
        } catch (e) {
        ...
        let teamStats = null;
        if (userTeam) {
          const teamKpis = allTeamKpis.filter(
            (k) =>
          ...
          teamKpis.forEach((k) => {
            const key =
              ...
              "";
            if (
              !teamLatestMap.has(key) ||
              (k.completed_month || 0) >
                (teamLatestMap.get(key).completed_month || 0)
            ) {
          ...
          teamLatestMap.forEach((k) => {
            ...
            if (
              nameKey === userName.toLowerCase().trim().replace(/\s+/g, " ")
            ) {
              userRecord = k;
            }
          });
          ...
          try {
            const teamMembers = Array.from(teamLatestMap.values())
              .map((k) => k.name?.trim())
              .filter(Boolean);
            const teamChannelsCount = await this.prisma.channel.count({
              where: {
                owner: { full_name: { in: teamMembers, mode: "insensitive" } },
              },
            });
          ...
          if (userRecord) {
            ...
            const individualKpi =
              currentMonthKpi ||
              (kpis.length > 0 ? kpis[kpis.length - 1] : null);
            teamStats = {
              teamName: userTeam || "Cá nhân",
              userVideo: individualKpi?.completed_month || 0,
              teamVideo:
                teamTotals.video || individualKpi?.completed_month || 0,
              userTraffic: Number(individualKpi?.traffic_month || 0),
              teamTraffic:
                teamTotals.traffic || Number(individualKpi?.traffic_month || 0),
              userRevenue: Number(individualKpi?.revenue_month || 0),
              teamRevenue:
                teamTotals.revenue || Number(individualKpi?.revenue_month || 0),
        ...
        if (todayReport?.answers) {
          ...
          if (ans && typeof ans === "object") {
            const checks = [
              ans["Bạn đã đăng video lên FB chưa?"] === true ||
                ans["Báo cáo Lark - Bạn đã đăng video lên FB chưa?"] === true,
              ans["Bạn đã đăng video lên IG chưa?"] === true ||
                ans["Báo cáo Lark - Bạn đã đăng video lên IG chưa?"] === true,
              ans["Bạn đã đăng video lên Tiktok chưa?"] === true ||
                ans["Báo cáo Lark - Bạn đã đăng video lên Tiktok chưa?"] ===
                  true,
              ans["Bạn đã đăng video lên Youtube chưa?"] === true ||
                ans["Báo cáo Lark - Bạn đã đăng video lên Youtube chưa?"] ===
                  true,
              ans[
                "Báo cáo Lark - Bạn đã báo cáo đầy đủ thông tin công việc trên lark chưa?"
              ] === true ||
              ...
              ans["Bạn đã check lại caption và hagtag video chưa?"] === true ||
                ans[
                  "Báo cáo Lark - Bạn đã check lại caption và hagtag video chưa?"
                ] === true,
            ];
        ...
        const userActivity = {
          ...
          avatar:
            this.convertDriveUrl(employee?.image_url) ||
            this.convertDriveUrl(currentMonthKpi?.link_image) ||
            this.convertDriveUrl(currentMonthKpi?.image_url) ||
            null,
          time: todayReport
            ? new Date(todayReport.date).toLocaleTimeString("vi-VN", {
                hour: "2-digit",
                minute: "2-digit",
              })
            : "Chưa báo cáo",
          ...
          traffic: Number(currentMonthKpi?.traffic_month || 0).toLocaleString(
            "vi-VN",
          ),
          revenue: Number(currentMonthKpi?.revenue_month || 0).toLocaleString(
            "vi-VN",
          ),
          reportStatus: todayReport ? "ĐÚNG HẠN" : "CHƯA BÁO CÁO",
          monthlyProgress:
            currentMonthKpi && currentMonthKpi.kpi_progress_month !== null
              ? Math.round(Number(currentMonthKpi.kpi_progress_month) * 100)
              : (currentMonthKpi?.kpi_month || 0) > 0
                ? Math.round(
                    ((currentMonthKpi?.completed_month || 0) /
                      currentMonthKpi.kpi_month) *
                      100,
                  )
                : 0,
        ...
        try {
          ...
          const [huykChannels, recentReports] = await Promise.all([
            ...
            this.prisma.larkReport.findMany({
              where: {
                name: {
                  in: allKpis.map((k) => k.name).filter(Boolean) as string[],
                },
              },
              orderBy: { date: "desc" },
            }),
          ...
          huykChannels.forEach((h) => {
            const ownerName = h.owner?.full_name;
            if (ownerName) {
          ...
          const activeKpis = allKpis.filter(
            (k) => k.state?.toLowerCase().trim() !== "off",
          );

          // Get team totals for contribution calculation
          const totalVideo = activeKpis.reduce(
            (sum, k) => sum + (k.completed_month || 0),
            0,
          );
          ...
          activeKpis.forEach((k) => {
            ...
            if (!latestMembers.has(key)) {
              const contribution =
                totalVideo > 0
                  ? Math.round(((k.completed_month || 0) / totalVideo) * 100)
                  : 0;
              ...
              if (report?.answers) {
                ...
                if (ans && typeof ans === "object") {
                  const checks = [
                    ans["Bạn đã đăng video lên FB chưa?"] === true ||
                      ans["Báo cáo Lark - Bạn đã đăng video lên FB chưa?"] ===
                        true,
                    ans["Bạn đã đăng video lên IG chưa?"] === true ||
                      ans["Báo cáo Lark - Bạn đã đăng video lên IG chưa?"] ===
                        true,
                    ans["Bạn đã đăng video lên Tiktok chưa?"] === true ||
                      ans[
                        "Báo cáo Lark - Bạn đã đăng video lên Tiktok chưa?"
                    ...
                    ans["Bạn đã check lại caption và hagtag video chưa?"] ===
                      true ||
                      ans[
                        "Báo cáo Lark - Bạn đã check lại caption và hagtag video chưa?"
                      ] === true,
        ...
        this.logger.error(
          `Error in getPersonalHistory for ${requesterEmail}: ${error.message}`,
          error.stack,
        );
        throw error;
      }
  ...
  private async getOutstandingFieldMap() {
    ...
    this.logger.log(
      `Field mapping for Outstanding table: ${JSON.stringify(result)}`,
    );
    return result;
  }
   ...
   */
  private async searchLarkRecords(
    baseId: string,
    tableId: string,
    filter: string,
  ): Promise<{ record_id: string; fields: any }[]> {
    const token = await this.getAccessToken();
    const url = `https://open.larksuite.com/open-apis/bitable/v1/apps/${baseId}/tables/${tableId}/records`;
  ...
  async pushUserChangesToLark(params: {
    ...
    this.logger.log(
      `[LarkSync] Pushing user changes to Lark Suite for: ${oldEmail || oldName}`,
    );
    ...
    try {
      const records = await this.searchLarkRecords(
        this.KPI_BASE_ID,
        this.KPI_TABLE_ID,
      ...
      await batchUpdate(
        this.KPI_BASE_ID,
        this.KPI_TABLE_ID,
        buildBatchPayload(records, updates),
      );
      this.logger.log(`[LarkSync] lark_kpi: updated ${records.length} records`);
    } catch (e) {
      this.logger.warn(`[LarkSync] lark_kpi sync failed: ${e.message}`);
    }
    ...
    try {
      ...
      this.logger.log(
        `[LarkSync] lark_employees: updated ${records.length} records`,
      );
    } catch (e) {
      this.logger.warn(`[LarkSync] lark_employees sync failed: ${e.message}`);
    }
    ...
    try {
      ...
      this.logger.log(
        `[LarkSync] lark_list_tasks: updated ${records.length} records`,
      );
    } catch (e) {
      this.logger.warn(`[LarkSync] lark_list_tasks sync failed: ${e.message}`);
    }

    this.logger.log(`[LarkSync] Done pushing user changes to Lark Suite`);
  }

  private async updateBitableRecord(
    baseId: string,
    tableId: string,
    recordId: string,
    fields: any,
    ...
    try {
      ...
      this.logger.error(
        `Failed to update Bitable record ${recordId} at ${url}`,
        error.response?.data || error.message,
      );
      ...
      if (error.response?.status === 404) {
        ...
        try {
          ...
          this.logger.error(
            `Batch update also failed for ${recordId}`,
            batchErr.response?.data || batchErr.message,
          );
          throw batchErr;
        }
      }
  ...
  async updateOutstandingStatus(
    ...
    try {
      ...
      return {
        success: true,
        message: "Status updated successfully (Local only)",
      };
    } catch (error) {
      this.logger.error(
        `Failed to update outstanding status for id ${id}`,
        error,
      );
      throw error;
    }
  ...
  async pushAllOutstandingData() {
    this.logger.log(
      "[LarkSync] pushAllOutstandingData disabled - this table is now independent.",
    );
  ...
  parseRecord(record: any) {
    ...
    const trangThai = String(
      fields["Trang Thai"] || fields["Trạng thái"] || "",
    ).toLowerCase();
    const isActive =
      trangThai !== "da nghi" &&
      trangThai !== "đã nghỉ" &&
  ...
  mapToUserRoles(role: string, team: string, position?: string): string[] {
    ...
    if (r.includes("ADMIN")) {
      ...
      leaderKeywords.some((key) => r.includes(key) || p.includes(key))
    ) {
      roles.push("LEADER");
    } else {
      roles.push("MEMBER");
  ...
  private rkReportAvatar(rk: any): string | null {
    ...
    if (Array.isArray(rk.image_url) && rk.image_url.length > 0) {
      ...
      return (
        rk.image_url[0].url ||
        rk.image_url[0].file_token ||
        rk.image_url[0].attachment_id ||
        null
      );
    }
    return null;
  ...
  async fetchListTaskRecords() {
    return this.fetchLarkRecordsGeneric(
      this.KPI_BASE_ID,
      this.LIST_TASK_TABLE_ID,
    );
  }
  ...
  private mapRecordToListTask(record: any) {
    ...
    const fileVoiceToken =
      Array.isArray(fileVoiceVal) && fileVoiceVal.length > 0
        ? fileVoiceVal[0].attachmentToken
        : null;
    ...
    return {
      ...
      employee_id: extractString(
        fields["ID Nhân viên"] || fields["ID Nhân Viên"],
      ),
      ...
      content_type:
        extractSingleSelect(fields["Tuyến Nội Dung"]) ||
        extractString(fields["Tuyến Nội Dung"]),
  ...
  async syncListTaskData() {
    try {
      ...
      this.logger.log(
        `Fetched ${records.length} ListTask records from Lark. Syncing...`,
      );
  ...
  async getDashboardAnalytics(filters?: {
    ...
    return this.cacheService.get(cacheKey, 3 * 60 * 1000, () =>
      this._buildDashboardAnalytics(filters),
    );
  }

  private async _buildDashboardAnalytics(filters?: {
    startDate?: string;
    endDate?: string;
    team?: string;
  }) {
    ...
    // Fetch everything
    const [tasks, allKpisInDb, usersWithChannels, allChannels] =
      await Promise.all([
        ...
        this.prisma.channel.findMany({
          where: {
            ...
            ...(teamFilter
              ? {
                  team: { name: { contains: teamFilter, mode: "insensitive" } },
                }
              : {}),
          },
          include: {
    ...
    const kpis = allKpisInDb.filter((k) => {
      ...
      if (!mStr) {
        ...
        return (
          ...
          monthsInRange.some(
            (m) =>
              rd.getMonth() + 1 === m.monthNum && rd.getFullYear() === m.year,
          )
        );
      }
      ...
      return monthsInRange.some((monthInfo) => {
        ...
        const mDigits = mStr.match(/\d+/g);
        return (
          mDigits && mDigits.some((d) => parseInt(d, 10) === monthInfo.monthNum)
        );
      });
    });
    ...
    // Maps channels by owner name (via FK relation)
    const channelsByOwnerMap = new Map<string, number>();
    allChannels.forEach((c) => {
      const ownerName = c.owner?.full_name;
      if (ownerName) {
        const ownerKey = ownerName.toLowerCase().trim().replace(/\s+/g, " ");
        channelsByOwnerMap.set(
          ownerKey,
          (channelsByOwnerMap.get(ownerKey) || 0) + 1,
        );
      }
    });
    ...
    const regionalChannelCounts = { vn: 0, global: 0 };
    allChannels.forEach((c) => {
      const region = getRegion(c.team?.name || "");
      regionalChannelCounts[region]++;
    });
    ...
    kpis.forEach((kpi) => {
      ...
      if (
        teamFilter &&
        !teamName.toLowerCase().includes(teamFilter) &&
        !teamFilter.includes(teamName.toLowerCase())
      ) {
        return;
      }
      ...
      if (empStats.has(key)) {
        ...
        const effectiveVideoCount =
          start.getTime() === end.getTime() && kpiDStr === targetDStr
            ? Number(kpi.completed_day) || 0
            : videosVal;
        ...
        empStats.set(key, {
          ...
          revenue: revenueVal,
          channels:
            channelsByOwnerMap.get(nameKey) ||
            user?._count.tracked_channels ||
            0,
          isLeader:
    ...
    tasks.forEach((task) => {
      ...
      const nameKey =
        task.employee_name?.toLowerCase().trim().replace(/\s+/g, " ") || "";
      const key = email || nameKey;
      ...
      if (empStats.has(key)) {
        ...
        if (
          teamFilter &&
          !resolvedTeam.toLowerCase().includes(teamFilter) &&
          !teamFilter.includes(resolvedTeam.toLowerCase())
        ) {
          return;
        }
    ...
    kpisForAggregation.forEach((stub) => {
      const key =
        stub.email?.toLowerCase().trim() ||
        stub.name?.toLowerCase().trim().replace(/\s+/g, " ");
      if (!key || empStats.has(key)) return; // Skip if already in stats
      const user = stub.email
        ? employeeMapByEmail.get(stub.email.toLowerCase().trim())
        : null;
      empStats.set(key, {
    ...
    const lineStatsMap: {
      [line: string]: { videoCount: number; traffic: number };
    } = {};
    empStats.forEach((stats) => {
      const totalTasks =
        Object.values(stats.lineCounts).reduce((a, b) => a + b, 0) || 1;
      Object.entries(stats.lineCounts).forEach(([line, count]) => {
        if (!lineStatsMap[line])
          lineStatsMap[line] = { videoCount: 0, traffic: 0 };
    ...
    const chartData = Object.entries(lineStatsMap)
      .map(([line, data]) => ({
        name: line,
    ...
    empStats.forEach((stats) => {
      ...
      if (!target.teamsBySlug[teamSlug]) {
        target.teamsBySlug[teamSlug] = {
          name: teamSlug,
          members: [],
          stats: { videos: 0, traffic: 0, revenue: 0, channels: 0 },
        };
      }
      ...
      const teams = Object.values(region.teamsBySlug).map((team: any) => ({
        ...
        members: team.members
          .map((m: any) => ({
            ...m,
            contribution:
              region.summary.videos > 0
                ? ((m.videoCount / region.summary.videos) * 100).toFixed(1) +
                  "%"
                : "0%",
          }))
          .sort(
    ...
    const prevTasksCount = await this.prisma.larkListTask.count({
      where: {
        ...
        ...(teamFilter
          ? { team: { contains: teamFilter, mode: "insensitive" } }
          : {}),
      },
    });

    const totalVideosInRange = Array.from(empStats.values()).reduce(
      (a, b) => a + Number(b.videoCount),
      0,
    );
    const totalTrafficInRange = Math.round(
      Array.from(empStats.values()).reduce((a, b) => a + b.traffic, 0),
    );
    const totalRevenueInRange = Math.round(
      Array.from(empStats.values()).reduce((a, b) => a + b.revenue, 0),
    );

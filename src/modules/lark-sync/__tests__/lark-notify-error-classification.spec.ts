...
describe('LarkNotifyService.sendMessage', () => {
  it('gửi được thì trả message_id', async () => {
    ...
    const kq = await dungService(post).sendMessage('ou_that', 'xin chào');

    expect(kq.messageId).toBe('om_abc');
  ...
  it('token dùng lại cho lần gửi sau, không xin lại mỗi tin', async () => {
    const post = jest.fn()
      .mockImplementationOnce(dapTokenOk)
      .mockImplementation(dapGuiOk);

    const service = dungService(post);
    await service.sendMessage('ou_a', 'tin 1');
    await service.sendMessage('ou_b', 'tin 2');

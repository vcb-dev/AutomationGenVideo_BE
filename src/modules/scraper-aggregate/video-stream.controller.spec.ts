...
describe('VideoStreamController — phát video qua trung gian', () => {
  ...
  it('xin được link trở lại thì xoá ngay mốc hết số dư, không bắt chờ hết 10 phút', async () => {
    ...
    await ctl.stream('douyin', 'v1', undefined, { headers: {} } as any, buildRes().res);
    const a = buildRes();
    ctl.trangThai(a.res);
    ...
    await ctl.stream('douyin', 'v2', undefined, { headers: {} } as any, buildRes().res);
    const b = buildRes();
    ctl.trangThai(b.res);

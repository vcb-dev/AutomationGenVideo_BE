...
describe('Channel URL Extractors & Scraper Controllers', () => {
  ...
  describe('Controller Validations (chặn lỗi 500 khi dán link sai/link video)', () => {
    ...
    beforeEach(() => {
      ...
      mockThreadsService = { scrapeByUsername: jest.fn().mockResolvedValue({ status: 'ok' }) };
    });

    it('DouyinScraperController: ném 400 Bad Request khi dán link video thay vì profile', async () => {
      const controller = new DouyinScraperController(mockDouyinService, {} as any);
      await expect(
      ...
      try {
        ...
        expect(err.getResponse().error).toContain('Không lấy được tài khoản Douyin');
      }
    ...
    it('ThreadsScraperController: bóc tách username từ URL threads.net/@username', async () => {
      const controller = new ThreadsScraperController(mockThreadsService, {} as any);
      const result = await controller.scrape({ username: 'https://www.threads.net/@zuck' });
      expect(result).toEqual({ status: 'ok' });
      expect(mockThreadsService.scrapeByUsername).toHaveBeenCalledWith('zuck', 50, { mode: undefined, days: undefined });
    });
  });
});
